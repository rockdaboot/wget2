...
static size_t default_page_length = 10;
...
static const unsigned char iri_ctype[256] = {
  [':'] = IRI_CTYPE_GENDELIM,
  ['/'] = IRI_CTYPE_GENDELIM,
  ['?'] = IRI_CTYPE_GENDELIM,
  ['#'] = IRI_CTYPE_GENDELIM,
...
int
wget_iri_supported (const wget_iri_t * iri)
{
  int it;
...
wget_iri_t *
wget_iri_parse (const char *url, const char *encoding)
{
  wget_iri_t *iri;
      ...
      if (*s == ':')
        {
          if (s[1])
            {
              if (!default_port
                  || (strcmp (s + 1, default_port)
                      && atoi (s + 1) != atoi (default_port)))
                iri->port = s + 1;
            }
        }
      *s = 0;
...
wget_iri_clone (wget_iri_t * iri)
{
  ...
  if (iri->host_allocated)
    ...
    clone->host =
      iri->host ? (char *) clone + (size_t) (iri->host -
                                             (const char *) iri) : NULL;

  clone->uri =
    iri->uri ? (char *) clone + (size_t) (iri->uri -
                                          (const char *) iri) : NULL;
  clone->display =
    iri->display ? (char *) clone + (size_t) (iri->display -
                                              (const char *) iri) : NULL;
  // not adjust scheme, it is a pointer to a static string
  clone->userinfo =
    iri->userinfo ? (char *) clone + (size_t) (iri->userinfo -
                                               (const char *) iri) : NULL;
  clone->password =
    iri->password ? (char *) clone + (size_t) (iri->password -
                                               (const char *) iri) : NULL;
  clone->port =
    iri->port ? (char *) clone + (size_t) (iri->port -
                                           (const char *) iri) : NULL;
  clone->resolv_port =
    iri->resolv_port ? (char *) clone + (size_t) (iri->resolv_port -
                                                  (const char *) iri) : NULL;
  clone->path =
    iri->path ? (char *) clone + (size_t) (iri->path -
                                           (const char *) iri) : NULL;
  clone->query =
    iri->query ? (char *) clone + (size_t) (iri->query -
                                            (const char *) iri) : NULL;
  clone->fragment =
    iri->fragment ? (char *) clone + (size_t) (iri->fragment -
                                               (const char *) iri) : NULL;
...
static char *
_iri_build_connection_part (wget_iri_t * iri)
{
  char *tag;
  size_t len;
  ...
  if (iri->port)
    {
      len =
        strlen (iri->scheme) + strlen (iri->host) + strlen (iri->port) + 4 +
        1;
      tag = xmalloc (len);
...
static size_t G_GNUC_WGET_NONNULL_ALL
_normalize_path (char *path)
{
...
const char *
wget_iri_relative_to_abs (wget_iri_t * base, const char *val, size_t len,
                          wget_buffer_t * buf)
{
  debug_printf ("*url = %.*s\n", (int) len, val);
  ...
  if (base)
    {
      ...
      iri =
        wget_iri_parse (wget_iri_relative_to_abs
                        (base, url, strlen (url), &buf), encoding);
  ...
  else
    {
      ...
      iri =
        wget_iri_parse (wget_iri_relative_to_abs
                        (NULL, url, strlen (url), NULL), encoding);
    }
...
int
wget_iri_compare (wget_iri_t * iri1, wget_iri_t * iri2)
{
  int n;
...
const char *
wget_iri_escape_path (const char *src, wget_buffer_t * buf)
{
  const char *begin;
...
const char *
wget_iri_escape_query (const char *src, wget_buffer_t * buf)
{
  const char *begin;
...
const char *
wget_iri_get_escaped_host (const wget_iri_t * iri, wget_buffer_t * buf)
{
  return wget_iri_escape (iri->host, buf);
}

const char *
wget_iri_get_escaped_resource (const wget_iri_t * iri, wget_buffer_t * buf)
{
  if (iri->path)
...
char *
wget_iri_get_path (const wget_iri_t * iri, wget_buffer_t * buf,
                   const char *encoding)
{
  if (buf->length)
    wget_buffer_memcat (buf, "/", 1);
...
char *
wget_iri_get_query_as_filename (const wget_iri_t * iri, wget_buffer_t * buf,
                                const char *encoding)
{
  if (iri->query)
    {
...
char *
wget_iri_get_filename (const wget_iri_t * iri, wget_buffer_t * buf,
                       const char *encoding)
{
  if (iri->path)
    {
