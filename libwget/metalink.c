...
typedef struct
{
  ...
  char hash[128], hash_type[16], location[8];
  long long length;
} _metalink_context_t;

static void
_free_mirror (wget_metalink_mirror_t * mirror)
{
  wget_iri_free (&mirror->iri);
}

static void
_metalink4_parse (void *context, int flags, const char *dir, const char *attr,
                  const char *val, size_t len, size_t pos G_GNUC_WGET_UNUSED)
{
  ...
  if (strncasecmp (dir, "/metalink/file", 14))
    return;
  ...
  if (attr)
    {
      if (*dir == 0)
      ...
      else if (!wget_strcasecmp_ascii (dir, "/url"))
        {
          ...
          else if (!wget_strcasecmp_ascii (attr, "priority")
                   || !wget_strcasecmp_ascii (attr, "preference"))
            {
          ...
          if (ctx->length && *ctx->hash_type && *ctx->hash)
            {
              ...
              strlcpy (piece.hash.type, ctx->hash_type,
                       sizeof (piece.hash.type));
              strlcpy (piece.hash.hash_hex, ctx->hash,
                       sizeof (piece.hash.hash_hex));

              piecep =
                wget_vector_get (metalink->pieces,
                                 wget_vector_size (metalink->pieces) - 1);
              if (piecep)
          ...
          if (*ctx->hash_type && *ctx->hash)
            {
              ...
              wget_vector_add (metalink->hashes, &hash,
                               sizeof (wget_metalink_hash_t));
            }
          ...
          if (!metalink->mirrors)
            {
              ...
              wget_vector_set_destructor (metalink->mirrors,
                                          (void (*)(void *)) _free_mirror);
            }
...
metalink4_parse (const char *xml)
{
  ...
  _metalink_context_t ctx = {.metalink = metalink,.priority =
                             999999,.location = "-" };
...
static void
_metalink3_parse (void *context, int flags, const char *dir, const char *attr,
                  const char *val, size_t len, size_t pos G_GNUC_WGET_UNUSED)
{
  _metalink_context_t *ctx = context;
  char value[len + 1];
  ...
  if (strncasecmp (dir, "/metalink/files/file", 20))
    return;
...
wget_metalink_t *
metalink3_parse (const char *xml)
{
  wget_metalink_t *metalink = xcalloc (1, sizeof (wget_metalink_t));
  _metalink_context_t ctx = {.metalink = metalink,.priority =
                             999999,.location = "-" };
...
_compare_mirror (wget_metalink_mirror_t ** m1, wget_metalink_mirror_t ** m2)
{
...
wget_metalink_sort_mirrors (wget_metalink_t * metalink)
{
  if (metalink)
    {
      wget_vector_setcmpfunc (metalink->mirrors,
