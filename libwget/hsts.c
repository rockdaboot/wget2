...
struct _wget_hsts_st
{
  ...
  int port;
  unsigned int include_subdomains:1;  // whether or not subdomains are included
};

static unsigned int G_GNUC_WGET_PURE
_hash_hsts (const wget_hsts_t * hsts)
{
...
_compare_hsts (const wget_hsts_t * h1, const wget_hsts_t * h2)
{
  int n;
...
wget_hsts_t *
wget_hsts_new (const char *host, int port, time_t maxage,
                      ...
                      int port)
{
  ...
  hsts.port = port;
  hsts.host = host;
  if ((hstsp = wget_hashmap_get (hsts_db->entries, &hsts))
      && hstsp->maxage >= now)
    return 1;
  ...
  for (p = host; (p = strchr (p, '.'));)
    {
      hsts.host = ++p;
      if ((hstsp = wget_hashmap_get (hsts_db->entries, &hsts))
          && hstsp->include_subdomains && hstsp->maxage >= now)
        return 1;
    }
...
wget_hsts_db_init (wget_hsts_db_t * hsts_db)
{
  ...
  hsts_db->entries =
    wget_hashmap_create (16, -2, (unsigned int (*)(const void *)) _hash_hsts,
                         (int (*)(const void *, const void *)) _compare_hsts);
  wget_hashmap_set_key_destructor (hsts_db->entries,
                                   (void (*)(void *)) wget_hsts_free);
  wget_hashmap_set_value_destructor (hsts_db->entries,
                                     (void (*)(void *)) wget_hsts_free);
      ...
      if (old)
          ...
          if (old->mtime < hsts->mtime)
            {
              ...
              debug_printf
                ("update HSTS %s:%d (maxage=%ld, includeSubDomains=%d)\n",
      ...
      else
        {
          ...
          debug_printf ("add HSTS %s:%d (maxage=%ld, includeSubDomains=%d)\n",
                        hsts->host, hsts->port, hsts->maxage,
...
static int G_GNUC_WGET_NONNULL_ALL
_hsts_save (FILE * fp, const wget_hsts_t * hsts)
{
  fprintf (fp, "%s %d %ld %d %ld\n", hsts->host, hsts->port, hsts->maxage,
           hsts->include_subdomains, hsts->mtime);
  return 0;
}
  ...
  if ((fp = fopen (fname, "w")))
    {
      ...
      wget_hashmap_browse (hsts_db->entries,
                           (int (*)(void *, const void *, void *)) _hsts_save,
                           fp);
