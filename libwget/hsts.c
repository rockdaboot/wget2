...
static int G_GNUC_WGET_NONNULL_ALL _hsts_save(const wget_hsts_t *hsts, FILE *fp)
{
...
static int _hsts_db_load(wget_hsts_db_t *hsts_db, FILE *fp)
{
	...
	// save open time to tell whether the file was modified later when saving
	hsts_db->load_time = (now != -1 ? now : 0);
...
static int _hsts_db_save(wget_hsts_db_t *hsts_db, FILE *fp)
{
	if (wget_hashmap_size(hsts_db->entries) > 0) {
...
int wget_hsts_db_save(wget_hsts_db_t *hsts_db, const char *fname)
{
	...
	if (!st.st_mtim.tv_sec || st.st_mtim.tv_sec > hsts_db->load_time)
		load_func = (int(*)(void *, FILE *))_hsts_db_load;

	ret = wget_update_file(fname, load_func, (int(*)(void *, FILE *))_hsts_db_save, hsts_db);
	if (ret < 0) {
		error_printf(_("Failed to write to HSTS file '%s' (%d)\n"), fname, errno);
	} else {
		// we called this twice because _hsts_db_save also called it
		// this one's for convenience
		// nothing particularly serious since it's a cheap function
