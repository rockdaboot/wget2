...
int wget_hsts_db_load(wget_hsts_db_t *hsts_db, const char *fname)
{
	...
	if (wget_update_file(fname, (wget_update_cb_t)_hsts_db_load, NULL, hsts_db)) {
...
static int _hsts_db_save(void *hsts_db, FILE *fp)
{
	...
	if (wget_hashmap_size(entries) > 0) {
		...
		wget_hashmap_browse(entries, (int(*)(void *, const void *, void *))_hsts_save, fp);
...
int wget_hsts_db_save(wget_hsts_db_t *hsts_db, const char *fname)
{
	...
	if (wget_update_file(fname, (wget_update_cb_t)_hsts_db_load, _hsts_db_save, hsts_db)) {
