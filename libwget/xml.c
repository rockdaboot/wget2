...
struct XML_CONTEXT
{
  const char *buf,    // pointer to original start of buffer (0-terminated)
    *p,        // pointer next char in buffer
    *token;      // token buffer
  int hints;      // XML_HINT...
  size_t token_size,    // size of token buffer
    token_len;      // used bytes of token buffer (not counting terminating 0 byte)
  void *user_ctx;    // user context (not needed if we were using nested functions)
...
getToken (XML_CONTEXT * context)
{
  ...
  if (c == '<')
    {        // fetch specials, e.g. start of comments '<!--'
      if (!(c = *context->p++))
        return NULL;
      if (c == '?' || c == '/')
        {
  ...
  if (c == '-')
    {        // fetch specials, e.g. end of comments '-->'
      if (!(c = *context->p++))
        return NULL;
      if (c != '-')
        {
...
static const char *
getScriptContent (XML_CONTEXT * context)
{
  ...
  if (context->callback)
    context->callback (context->user_ctx, XML_FLG_CONTENT | XML_FLG_END,
                       "script", NULL, context->token, context->token_len,
                       context->token - context->buf);

  return context->token;
}

static const char *
getUnparsed (XML_CONTEXT * context, int flags, const char *end, size_t len,
             const char *directory)
{
  ...
  if (len == 1)
    {
      for (context->token = context->p; (c = *context->p) && c != *end;
           context->p++);
    }
  else
    {
      for (context->token = context->p; (c = *context->p); context->p++)
        {
          if (c == *end && context->p[1] == end[1]
              && (len == 2 || context->p[2] == end[2]))
            {
              break;
            }
        }
  ...
  */
  if (context->callback)
    context->callback (context->user_ctx, flags, directory, NULL,
                       context->token, context->token_len,
                       context->token - context->buf);
...
static const char *
getComment (XML_CONTEXT * context)
{
  return getUnparsed (context, XML_FLG_COMMENT, "-->", 3, NULL);
}

static const char *
getProcessing (XML_CONTEXT * context)
{
  return getUnparsed (context, XML_FLG_PROCESSING, "?>", 2, NULL);
}

static const char *
getSpecial (XML_CONTEXT * context)
{
  return getUnparsed (context, XML_FLG_SPECIAL, ">", 1, NULL);
}

static const char *
getContent (XML_CONTEXT * context, const char *directory)
{
  int c;

  for (context->token = context->p; (c = *context->p) && c != '<';
       context->p++);
  ...
  if (context->callback && context->token_len)
    context->callback (context->user_ctx, XML_FLG_CONTENT, directory, NULL,
                       context->token, context->token_len,
                       context->token - context->buf);
...
static void
parseXML (const char *dir, XML_CONTEXT * context)
{
  const char *tok;
  char directory[256] = "";
  size_t pos = 0;
  ...
  do
    {
      ...
      if (context->token_len)
        debug_printf ("%s=%.*s\n", directory, (int) context->token_len,
                      context->token);

      if (!(tok = getToken (context)))
        return;
          ...
          if (!(context->hints & XML_HINT_HTML))
            {
              if (!pos || directory[pos - 1] != '/')
                snprintf (&directory[pos], sizeof (directory) - pos, "/%.*s",
                          (int) context->token_len, tok);
              else
                snprintf (&directory[pos], sizeof (directory) - pos, "%.*s",
                          (int) context->token_len, tok);
            }
          ...
          while ((tok = getToken (context)))
            {
              ...
              if (context->token_len == 2 && !strncmp (tok, "/>", 2))
                {
                  if (context->callback)
                    context->callback (context->user_ctx, flags | XML_FLG_END,
                                       directory, NULL, NULL, 0, 0);
                  break;  // stay in this level
                }
              else if (context->token_len == 1 && *tok == '>')
                {
                  if (context->callback)
                    context->callback (context->user_ctx,
                                       flags | XML_FLG_CLOSE, directory, NULL,
                                       NULL, 0, 0);
                  if (context->hints & XML_HINT_HTML)
                    {
          ...
          if (!strncmp (tok, "</", 2))
            {
              ...
              if (context->callback)
                {
                  if (!(context->hints & XML_HINT_HTML))
                    context->callback (context->user_ctx, XML_FLG_END,
                                       directory, NULL, NULL, 0, 0);
                  else
                    {
                      ...
                      context->callback (context->user_ctx, XML_FLG_END, tag,
                                         NULL, NULL, 0, 0);
                    }
                }
              if (!(tok = getToken (context)))
      ...
      else if (context->token_len == 4 && !strncmp (tok, "<!--", 4))
        {      // comment - ignore
          getComment (context);
          debug_printf ("%s=<!--%.*s-->\n", directory,
                        (int) context->token_len, context->token);
