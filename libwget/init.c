...
static struct _CONFIG
{
  ...
  char cookies_enabled, keep_session_cookies;
} _config =
  {
    .cookies_enabled = 0};
...
void
wget_global_init (int first_key, ...)
{
  va_list args;
  int key;
  ...
  for (key = first_key; key; key = va_arg (args, int))
    {
      switch (key)
        ...
        case WGET_DEBUG_STREAM:
          wget_logger_set_stream (wget_get_logger (WGET_LOGGER_DEBUG),
                                  va_arg (args, FILE *));
          break;
        ...
        case WGET_DEBUG_FILE:
          wget_logger_set_file (wget_get_logger (WGET_LOGGER_DEBUG),
        ...
        case WGET_ERROR_FUNC:
          wget_logger_set_func (wget_get_logger (WGET_LOGGER_ERROR),
                                va_arg (args,
                                        void (*)(const char *, size_t)));
          break;
        case WGET_ERROR_FILE:
          wget_logger_set_file (wget_get_logger (WGET_LOGGER_ERROR),
                                va_arg (args, const char *));
          break;
        case WGET_INFO_STREAM:
          wget_logger_set_stream (wget_get_logger (WGET_LOGGER_INFO),
                                  va_arg (args, FILE *));
          break;
        case WGET_INFO_FUNC:
          ...
          break;
        case WGET_INFO_FILE:
          wget_logger_set_file (wget_get_logger (WGET_LOGGER_INFO),
                                va_arg (args, const char *));
          break;
...
wget_global_get_ptr (int key)
{
  switch (key)
    ...
    case WGET_DEBUG_FUNC:
      return (void *)
        wget_logger_get_func (wget_get_logger (WGET_LOGGER_DEBUG));
    case WGET_DEBUG_FILE:
    ...
    case WGET_ERROR_FUNC:
      return (void *)
        wget_logger_get_func (wget_get_logger (WGET_LOGGER_ERROR));
    case WGET_ERROR_FILE:
    ...
    case WGET_INFO_FUNC:
      return (void *)
        wget_logger_get_func (wget_get_logger (WGET_LOGGER_INFO));
    case WGET_INFO_FILE:
