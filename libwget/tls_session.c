...
wget_tls_session_db *wget_tls_session_db_init(wget_tls_session_db *tls_session_db)
{
	...
	tls_session_db->entries = wget_hashmap_create(16, (wget_hashmap_hash_fn *) _hash_tls_session, (wget_hashmap_compare_fn *) _compare_tls_session);
...
int wget_tls_session_db_load(wget_tls_session_db *tls_session_db, const char *fname)
{
	...
	if (wget_update_file(fname, (wget_update_load_fn *) _tls_session_db_load, NULL, tls_session_db)) {
...
static int _tls_session_db_save(void *tls_session_db, FILE *fp)
{
	...
	if (wget_hashmap_size(entries) > 0) {
		...
		wget_hashmap_browse(entries, (wget_hashmap_browse_fn *) _tls_session_save, fp);
...
int wget_tls_session_db_save(wget_tls_session_db *tls_session_db, const char *fname)
{
	...
	if (wget_update_file(fname, (wget_update_load_fn *) _tls_session_db_load, _tls_session_db_save, tls_session_db)) {
