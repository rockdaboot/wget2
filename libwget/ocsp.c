...
{
  wget_hashmap_t *fingerprints;
  wget_hashmap_t *hosts;
...
{
  const char *key;
  time_t maxage;    // expiry time
  time_t mtime;    // creation time
  int valid;      // 1=valid, 0=revoked
};

static unsigned int G_GNUC_WGET_PURE
_hash_ocsp (const wget_ocsp_t * ocsp)
{
  unsigned int hash = 0;
...
_compare_ocsp (const wget_ocsp_t * h1, const wget_ocsp_t * h2)
{
...
wget_ocsp_fingerprint_in_cache (const wget_ocsp_db_t * ocsp_db,
                                const char *fingerprint, int *revoked)
{
  if (ocsp_db)
    {
      ...
      if ((ocspp = wget_hashmap_get (ocsp_db->fingerprints, &ocsp))
          && ocspp->maxage >= time (NULL))
        {
          if (revoked)
            *revoked = !ocspp->valid;
          return 1;
...
int
wget_ocsp_hostname_is_valid (const wget_ocsp_db_t * ocsp_db,
                             const char *hostname)
{
  if (ocsp_db)
    {
      ...
      // look for an exact match
      ocsp.key = hostname;
      if ((ocspp = wget_hashmap_get (ocsp_db->hosts, &ocsp))
          && ocspp->maxage >= time (NULL))
        {
          return 1;
        }
...
wget_ocsp_db_init (wget_ocsp_db_t * ocsp_db)
{
  ...
  ocsp_db->fingerprints =
    wget_hashmap_create (16, -2, (unsigned int (*)(const void *)) _hash_ocsp,
                         (int (*)(const void *, const void *)) _compare_ocsp);
  wget_hashmap_set_key_destructor (ocsp_db->fingerprints,
                                   (void (*)(void *)) wget_ocsp_free);
  wget_hashmap_set_value_destructor (ocsp_db->fingerprints,
                                     (void (*)(void *)) wget_ocsp_free);

  ocsp_db->hosts =
    wget_hashmap_create (16, -2, (unsigned int (*)(const void *)) _hash_ocsp,
                         (int (*)(const void *, const void *)) _compare_ocsp);
  wget_hashmap_set_key_destructor (ocsp_db->hosts,
                                   (void (*)(void *)) wget_ocsp_free);
  wget_hashmap_set_value_destructor (ocsp_db->hosts,
                                     (void (*)(void *)) wget_ocsp_free);
...
static int
_ocsp_db_load (wget_ocsp_db_t * ocsp_db, const char *fname, int load_hosts)
{
  wget_ocsp_t ocsp;
  FILE *fp;
  ...
  if ((fp = fopen (fname, "r")))
    {
      while ((buflen = wget_getline (&buf, &bufsize, fp)) >= 0)
        {
          ...
          if (ok)
            {
              if (load_hosts)
                wget_ocsp_db_add_host (ocsp_db,
                                       wget_memdup (&ocsp, sizeof (ocsp)));
              else
                wget_ocsp_db_add_fingerprint (ocsp_db,
                                              wget_memdup (&ocsp,
                                                           sizeof (ocsp)));
            }
          else
      ...
      nentries =
        wget_hashmap_size (load_hosts ? ocsp_db->hosts : ocsp_db->
                           fingerprints);

      debug_printf (_("have %d OCSP %s%s in cache\n"), nentries,
                    load_hosts ? "host" : "fingerprint",
                    nentries != 1 ? "s" : "");
    }
...
wget_ocsp_db_load (wget_ocsp_db_t * ocsp_db, const char *fname)
{
  ...
  return _ocsp_db_load (ocsp_db, fname, 0) + _ocsp_db_load (ocsp_db,
                                                            fname_hosts, 1);
}

static int G_GNUC_WGET_NONNULL_ALL
_ocsp_save_entry (FILE * fp, const wget_ocsp_t * ocsp)
{
...
static int G_GNUC_WGET_NONNULL_ALL
_ocsp_save_host (FILE * fp, const wget_ocsp_t * ocsp)
{
  fprintf (fp, "%s %ld %ld\n", ocsp->key, ocsp->maxage, ocsp->mtime);
  return 0;
}
...
static int
_ocsp_db_save (wget_hashmap_t * map, const char *fname, int save_hosts)
{
  FILE *fp;
  int ret = -1, size;
  ...
  if ((fp = fopen (fname, "w")))
    {
      fputs ("#OCSP 1.0 file\n", fp);
      fputs ("#Generated by Wget " PACKAGE_VERSION
             ". Edit at your own risk.\n", fp);
      if (save_hosts)
        {
          fputs ("<hostname> <time_t maxage> <time_t mtime>\n\n", fp);
          wget_hashmap_browse (map,
