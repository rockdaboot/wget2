...
static struct _config
{
  ...
  *secure_protocol,
    *direct_options,
    *ca_directory,
    *ca_file, *cert_file, *key_file, *crl_file, *ocsp_server, *alpn;
  wget_ocsp_db_t * ocsp_cert_cache, *ocsp_host_cache;
  char
  check_certificate,
    check_hostname,
    ca_type, cert_type, key_type, print_info, ocsp, ocsp_stapling;
} _config =
  {
# ifdef HAVE_GNUTLS_OCSP_H
    .check_certificate = 1,.ocsp_stapling = 1,
# endif
    .ca_type = WGET_SSL_X509_FMT_PEM,.cert_type =
    WGET_SSL_X509_FMT_PEM,.key_type =
    WGET_SSL_X509_FMT_PEM,.secure_protocol = "AUTO",.ca_directory = "system"};

struct _session_context
{
  const char *hostname;
  char ocsp_stapling, valid;
};

static gnutls_certificate_credentials_t _credentials;
static gnutls_priority_t _priority_cache;
...
wget_ssl_set_config_string (int key, const char *value)
{
  switch (key)
    ...
    case WGET_SSL_SECURE_PROTOCOL:
      ...
      break;
    case WGET_SSL_DIRECT_OPTIONS:
      _config.direct_options = value;
      break;
...
wget_ssl_set_config_int (int key, int value)
{
  switch (key)
    ...
    case WGET_SSL_CHECK_CERTIFICATE:
      _config.check_certificate = (char) value;
      break;
    case WGET_SSL_CHECK_HOSTNAME:
      _config.check_hostname = (char) value;
      break;
    ...
    case WGET_SSL_PRINT_INFO:
      _config.print_info = (char) value;
      break;
    case WGET_SSL_OCSP:
      _config.ocsp = (char) value;
      break;
    case WGET_SSL_OCSP_STAPLING:
      _config.ocsp_stapling = (char) value;
      break;
    default:
      error_printf (_
...
static void
_print_x509_certificate_info (gnutls_session_t session)
{
  char dn[128];
  ...
  for (ncert = 0; ncert < cert_list_size; ncert++)
    {
      if ((cert_type =
           gnutls_certificate_type_get (session)) == GNUTLS_CRT_X509)
        {
          ...
          info_printf (_("  Certificate is valid since: %s"),
                       ctime (&activet));
          ...
          if (!gnutls_fingerprint
              (GNUTLS_DIG_MD5, &cert_list[ncert], digest, &digest_size))
            {
              char digest_hex[digest_size * 2 + 1];

              wget_memtohex (digest, digest_size, digest_hex,
                             sizeof (digest_hex));
          ...
          if (!gnutls_x509_crt_get_serial (cert, serial, &serial_size))
            {
              ...
              wget_memtohex (digest, digest_size, serial_hex,
                             sizeof (serial_hex));
...
static int
_print_info (gnutls_session_t session)
{
  const char *tmp;
...
static int
send_ocsp_request (const char *server,
                   ...
                   wget_buffer_t ** ocsp_data, gnutls_datum_t * nonce)
{
      ...
      if (rc < 0)
        {
          ...
          do
            {
              rc =
                gnutls_x509_crt_get_authority_info_access (issuer, i++,
                                                           GNUTLS_IA_OCSP_URI,
                                                           &data, NULL);
            }
          while (rc < 0 && rc != GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE);
  ...
  if ((rc = wget_http_open (&conn, iri)) == WGET_E_SUCCESS)
    {
      if (wget_http_send_request_with_body (conn, req, body.data, body.size)
          == 0)
        {
...
static void
print_ocsp_verify_res (unsigned int status)
{
                     ...
                     gnutls_datum_t * nonce)
{
  ...
  rc = gnutls_ocsp_resp_import (resp, &(gnutls_datum_t)
                                {
                                  .data = (unsigned char *) data->data,.size =
                                    data->length});
  if (rc < 0)
    {
...
#  endif
  ...
  if ((rc = gnutls_ocsp_resp_verify_direct (resp, issuer, &status, 0)) < 0)
    {
      debug_printf ("gnutls_ocsp_resp_verify_direct: %s",
                    gnutls_strerror (rc));
      goto cleanup;
    }
 ...
 finish_ok:
  debug_printf ("OCSP server flags certificate not revoked as of %s",
                ctime (&vtime));
  ret = 1;
 ...
 */
static char *
_get_cert_fingerprint (gnutls_x509_crt_t cert, char *fingerprint_hex,
                       size_t length)
{
  ...
  if ((err =
       gnutls_x509_crt_get_fingerprint (cert, GNUTLS_DIG_SHA256, fingerprint,
                                        &fingerprint_size)) < 0)
    {
 ...
 */
static void
_add_cert_to_ocsp_cache (gnutls_x509_crt_t cert, int valid)
{
...
static int
cert_verify_ocsp (gnutls_x509_crt_t cert, gnutls_x509_crt_t issuer)
{
...
# if GNUTLS_VERSION_NUMBER >= 0x030104
  if (gnutls_certificate_verify_peers3 (session, hostname, &status) !=
      GNUTLS_E_SUCCESS)
    {
# else
...
# endif
          ...
          if (status &
              ~(GNUTLS_CERT_INVALID | GNUTLS_CERT_REVOKED |
                ...
                GNUTLS_CERT_INSECURE_ALGORITHM | GNUTLS_CERT_NOT_ACTIVATED |
                GNUTLS_CERT_EXPIRED
# ifdef GNUTLS_CERT_SIGNATURE_FAILURE
...
# endif
      ...
      if (_config.check_hostname && hostname
...
#  if GNUTLS_VERSION_NUMBER >= 0x030103
              if (gnutls_ocsp_status_request_is_checked (session, 0))
                {
                  info_printf
                    ("Server certificate is valid regarding OCSP stapling\n");
          ...
          for (unsigned it = nvalid; it < cert_list_size; it++)
            {
              ...
              if (wget_ocsp_fingerprint_in_cache
                  (_config.ocsp_cert_cache, fingerprint, &revoked))
                {
                  // found cert's fingerprint in cache
                  if (revoked)
                    {
                      debug_printf
                        ("Certificate[%u] of '%s' has been revoked (cached)\n",
                         it, hostname);
                      nrevoked++;
                    }
                  else
              ...
              if ((err =
                   gnutls_certificate_get_issuer (_credentials, cert, &issuer,
                                                  0)) != GNUTLS_E_SUCCESS
                  && it < cert_list_size - 1)
                {
              ...
              if (ocsp_ok == 1)
                ...
                {
                  debug_printf (_
                                ("%s: Certificate[%u] of '%s' has been revoked (via OCSP)\n"),
                                tag, it, hostname);
...
# endif
  ...
  static void
    _set_credentials (gnutls_certificate_credentials_t * credentials)
  {
    ...
    if (_config.cert_file && _config.key_file)
      {
        ...
        if (gnutls_certificate_set_x509_key_file
            (*credentials, _config.cert_file, _config.key_file,
             _key_type (_config.key_type)) != GNUTLS_E_SUCCESS)
    ...
    if (_config.ca_file)
      {
        if (gnutls_certificate_set_x509_trust_file
            (*credentials, _config.ca_file, _key_type (_config.ca_type)) <= 0)
    ...
    if (!_init)
      {
        ...
        gnutls_certificate_set_verify_function (_credentials,
                                                _verify_certificate_callback);

        if (_config.ca_directory && *_config.ca_directory
            && _config.check_certificate)
          {
...
# endif
                    ...
                    while ((dp = readdir (dir)))
                      {
                        ...
                        if (len >= 4
                            && !strncasecmp (dp->d_name + len - 4, ".pem", 4))
                          {
                            struct stat st;
                            char fname[dirlen + 1 + len + 1];

                            snprintf (fname, sizeof (fname), "%s/%s",
                                      _config.ca_directory, dp->d_name);
        ...
        if (_config.crl_file)
          {
            ...
            if ((rc =
                 gnutls_certificate_set_x509_crl_file (_credentials,
                                                       _config.crl_file,
                                                       GNUTLS_X509_FMT_PEM)) <=
                0)
        ...
        if (_config.secure_protocol || _config.direct_options)
          {
            ...
            if (_config.direct_options)
              {
                ret =
                  gnutls_priority_init (&_priority_cache,
                                        _config.direct_options, NULL);
              }
            else if (!wget_strcasecmp_ascii (_config.secure_protocol, "PFS"))
              {
                ...
                if ((ret =
                     gnutls_priority_init (&_priority_cache, priorities,
            ...
            else
              {
                if (!wget_strncasecmp_ascii (_config.secure_protocol, "SSL", 3))
                  ...
                  if (!wget_strcasecmp_ascii (_config.secure_protocol, "TLSv1"))
  ...
  static int
    _do_handshake (gnutls_session_t session, int sockfd, int timeout)
  {
    int ret;
...
# endif
    ...
    struct _session_context *ctx =
      wget_calloc (1, sizeof (struct _session_context));
    ...
    if (hostname)
      {
        if (!
            (ctx->valid =
             wget_ocsp_hostname_is_valid (_config.ocsp_host_cache, hostname)))
          {
#  if GNUTLS_VERSION_NUMBER >= 0x030103
            if ((rc =
...
# endif
  ...
  static gnutls_certificate_credentials_t _server_credentials;
  static gnutls_priority_t _server_priority_cache;
    ...
    if (!_server_init)
      {
        ...
        */
        if ((ret =
             gnutls_priority_init (&_server_priority_cache, "PERFORMANCE",
                                   NULL)) < 0)
    ...
    for (;;)
      {
        if (gnutls_record_check_pending (session) <= 0 &&
            (rc =
             wget_ready_2_read ((int) (ptrdiff_t)
                                gnutls_transport_get_ptr (session),
                                timeout)) <= 0)
          return rc;
...
# endif
                            ...
                            int timeout)
  {
    ...
    for (;;)
      {
        if ((rc =
             wget_ready_2_write ((int) (ptrdiff_t)
                                 gnutls_transport_get_ptr (session),
                                 timeout)) <= 0)
          return rc;
