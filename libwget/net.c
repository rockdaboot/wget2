...
static wget_vector_t * dns_cache;
static wget_thread_mutex_t dns_mutex = WGET_THREAD_MUTEX_INITIALIZER;

static struct addrinfo *
_wget_dns_cache_get (const char *host, const char *port)
{
  if (dns_cache)
    {
      ...
      entryp =
        wget_vector_get (dns_cache, wget_vector_find (dns_cache, &entry));
...
_compare_addr (struct ADDR_ENTRY *a1, struct ADDR_ENTRY *a2)
{
  int n;
...
static void
_free_dns (struct ADDR_ENTRY *entry)
{
  freeaddrinfo (entry->addrinfo);
}

static struct addrinfo *
_wget_dns_cache_add (const char *host, const char *port,
                     struct addrinfo *addrinfo)
{
  // insert addrinfo into dns cache
  size_t hostlen = host ? strlen (host) + 1 : 1;
  size_t portlen = port ? strlen (port) + 1 : 1;
  struct ADDR_ENTRY *entryp =
    xmalloc (sizeof (struct ADDR_ENTRY) + hostlen + portlen);
  int index;
...
struct addrinfo *
wget_tcp_resolve (wget_tcp_t * tcp, const char *host, const char *port)
{
  ...
  if (!isdigit (*port))
    {
      if (!wget_strcasecmp_ascii (port, "http"))
        port = "80";
      else if (!wget_strcasecmp_ascii (port, "https"))
        port = "443";
      else if (!wget_strcasecmp_ascii (port, "ftp"))
        port = "21";
      else
        {
  ...
  for (tries = 0; tries < 3; tries++)
    {
      if ((rc = getaddrinfo (host, port, &hints, &addrinfo)) == 0
          || rc != EAI_AGAIN)
        break;
      ...
      for (ai = addrinfo; ai; ai = ai->ai_next)
        {
          ...
          if ((rc =
               getnameinfo (ai->ai_addr, ai->ai_addrlen, adr, sizeof (adr),
                            sport, sizeof (sport),
...
static int G_GNUC_WGET_CONST
_value_to_family (int value)
{
  switch (value)
    {
...
int
wget_tcp_get_dns_caching (wget_tcp_t * tcp)
{
...
int
wget_tcp_get_preferred_family (wget_tcp_t * tcp)
{
...
int
wget_tcp_get_family (wget_tcp_t * tcp)
{
  ...
  if (tcp)
    {
      ...
      if (getsockname (tcp->sockfd, addr, &addr_len) == 0)
        {
          if (getnameinfo
              (addr, addr_len, NULL, 0, s_port, sizeof (s_port),
               NI_NUMERICSERV) == 0)
            return atoi (s_port);
        }
    }
...
int
wget_tcp_get_timeout (wget_tcp_t * tcp)
{
...
int
wget_tcp_get_ssl (wget_tcp_t * tcp)
{
...
const char *
wget_tcp_get_ssl_hostname (wget_tcp_t * tcp)
{
...
void
wget_tcp_deinit (wget_tcp_t ** _tcp)
{
  wget_tcp_t *tcp;
  ...
  for (ai = tcp->addrinfo; ai; ai = ai->ai_next)
    {
      ...
      if ((sockfd =
           socket (ai->ai_family, ai->ai_socktype, ai->ai_protocol)) != -1)
        {
          ...
          if (setsockopt
              (sockfd, SOL_SOCKET, SO_REUSEADDR, (void *) &on,
               sizeof (on)) == -1)
            error_printf (_("Failed to set socket option REUSEADDR\n"));

          on = 1;
          if (setsockopt
              (sockfd, IPPROTO_TCP, TCP_NODELAY, (void *) &on,
               sizeof (on)) == -1)
          ...
          if (tcp->bind_addrinfo)
            {
              if (wget_get_logger (WGET_LOGGER_DEBUG)->vprintf)
                {
                  if ((rc =
                       getnameinfo (tcp->bind_addrinfo->ai_addr,
                                    tcp->bind_addrinfo->ai_addrlen, adr,
                                    sizeof (adr), s_port, sizeof (s_port),
                                    NI_NUMERICHOST | NI_NUMERICSERV)) == 0)
              ...
              if (bind
                  (sockfd, tcp->bind_addrinfo->ai_addr,
                   tcp->bind_addrinfo->ai_addrlen) != 0)
                {
...
int
wget_tcp_listen (wget_tcp_t * tcp, const char *host, const char *port,
                 int backlog)
{
  struct addrinfo *ai;
  int sockfd = -1, rc;
          ...
          if (tcp->tcp_fastopen)
            {
              on = 1;
              if (setsockopt
                  (sockfd, IPPROTO_TCP, TCP_FASTOPEN, &on, sizeof (on)) == -1)
          ...
          if (listen (sockfd, backlog) == 0)
            {
              ...
              if (wget_get_logger (WGET_LOGGER_DEBUG)->vprintf)
                {
                  if (!port)
                    snprintf (s_port, sizeof (s_port), "%d",
                              wget_tcp_get_local_port (tcp));

                  if ((rc =
                       getnameinfo (ai->ai_addr, ai->ai_addrlen, adr,
                                    sizeof (adr), NULL, 0,
                                    NI_NUMERICHOST)) == 0)
                    ...
                    debug_printf ("%ssecure listen on %s:%s (%s)...\n",
                                  tcp->ssl ? "" : "in", host,
                                  port ? port : s_port, gai_strerror (rc));
                }
...
wget_tcp_accept (wget_tcp_t * parent_tcp)
{
  ...
  if ((sockfd =
       accept (parent_tcp->sockfd, parent_tcp->bind_addrinfo->ai_addr,
               &parent_tcp->bind_addrinfo->ai_addrlen)) != -1)
    {
      wget_tcp_t *tcp = xmalloc (sizeof (wget_tcp_t));
...
wget_tcp_write (wget_tcp_t * tcp, const char *buf, size_t count)
{
  ...
  if (tcp->ssl_session)
    return wget_ssl_write_timeout (tcp->ssl_session, buf, count,
                                   tcp->timeout);
          ...
          if (n < 0 && errno == EOPNOTSUPP)
            {
              ...
              tcp->tcp_fastopen = 0;
              rc =
                connect (tcp->sockfd, tcp->connect_addrinfo->ai_addr,
                         tcp->connect_addrinfo->ai_addrlen);
              if (rc < 0 && errno != EAGAIN && errno != ENOTCONN
# ifdef EINPROGRESS
                  && errno != EINPROGRESS
# endif
                  )
