...
static void
_rss_get_url (void *context, int flags, const char *dir, const char *attr,
              const char *val, size_t len, size_t pos G_GNUC_WGET_UNUSED)
{
  ...
  if ((flags & XML_FLG_ATTRIBUTE))
    {
      if (!wget_strcasecmp_ascii (attr, "url")
          || !wget_strcasecmp_ascii (attr, "href")
          || !wget_strcasecmp_ascii (attr, "src")
          || !wget_strcasecmp_ascii (attr, "domain")
      ...
      if (elem)
        {
          ...
          if (!wget_strcasecmp_ascii (elem, "guid")
              || !wget_strcasecmp_ascii (elem, "link")
              || !wget_strcasecmp_ascii (elem, "comments")
              || !wget_strcasecmp_ascii (elem, "docs"))
            {
              for (; len && isspace (*val); val++, len--);  // skip leading spaces
              for (; len && isspace (val[len - 1]); len--);  // skip trailing spaces
...
wget_rss_get_urls_inline (const char *rss, wget_vector_t ** urls)
{
  ...
  wget_xml_parse_buffer (rss, _rss_get_url, &context,
