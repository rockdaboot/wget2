...
#ifdef WITH_LIBPSL
  psl_ctx_t * psl;    // libpsl Publix Suffix List context
#endif
...
_compare_cookie (const wget_cookie_t * c1, const wget_cookie_t * c2)
{
  int n;
...
static int G_GNUC_WGET_NONNULL_ALL
_domain_match (const char *domain, const char *host)
{
...
G_GNUC_WGET_NONNULL ((1))
  _path_match (const char *cookie_path, const char *request_path)
{
  ...
  if (!request_path || *request_path != '/'
      || !(last_slash = strrchr (request_path + 1, '/')))
    {
      request_path = "/";
      iri_path_length = 1;
    }
...
static int
_wget_cookie_normalize_cookie (const wget_iri_t * iri, wget_cookie_t * cookie)
{
  /*
...
int
wget_cookie_check_psl (const wget_cookie_db_t * cookie_db,
...
int
wget_cookie_store_cookie (wget_cookie_db_t * cookie_db,
                          wget_cookie_t * cookie)
{
  ...
  old = wget_vector_get (cookie_db->cookies, pos =
                         wget_vector_find (cookie_db->cookies, cookie));
...
void
wget_cookie_store_cookies (wget_cookie_db_t * cookie_db,
                           wget_vector_t * cookies)
{
  if (cookie_db)
    {
...
char *
wget_cookie_create_request_header (wget_cookie_db_t * cookie_db,
  ...
  for (it = 0; it < wget_vector_size (cookie_db->cookies); it++)
    {
      ...
      if (((!cookie->host_only && _domain_match (cookie->domain, iri->host))
          ...
          && _path_match (cookie->path, iri->path))
        {
          ...
          if (buf.length)
            wget_buffer_printf_append2 (&buf, "; %s=%s", cookie->name,
                                        cookie->value);
          else
            wget_buffer_printf_append2 (&buf, "%s=%s", cookie->name,
                                        cookie->value);
...
wget_cookie_db_init (wget_cookie_db_t * cookie_db)
{
  ...
  cookie_db->cookies =
    wget_vector_create (32, -2,
                        (int (*)(const void *, const void *))
                        _compare_cookie);
  wget_vector_set_destructor (cookie_db->cookies,
                              (void (*)(void *)) wget_cookie_deinit);
  ...
  if ((fp = fopen (fname, "w")))
    {
      ...
      for (it = 0; it < wget_vector_size (cookie_db->cookies) && !ferror (fp);
           it++)
        {
          wget_cookie_t *cookie = wget_vector_get (cookie_db->cookies, it);
          ...
          fprintf (fp, "%s%s%s\t%s\t%s\t%s\t%" PRId64 "\t%s\t%s\n", cookie->http_only ? "#HttpOnly_" : "", cookie->domain_dot ? "." : "",  // compatibility, irrelevant since RFC 6562
                   cookie->domain,
                   cookie->host_only ? "FALSE" : "TRUE",
                   cookie->path, cookie->secure_only ? "TRUE" : "FALSE",
                   (int64_t) cookie->expires, cookie->name, cookie->value);
        }
      ...
      while ((buflen = wget_getline (&buf, &bufsize, fp)) >= 0)
        {
          ...
          if (wget_cookie_normalize (NULL, &cookie) == 0
              && wget_cookie_check_psl (cookie_db, &cookie) == 0)
            {
              ncookies++;
