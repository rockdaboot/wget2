...
static const unsigned char http_ctype[256] = {
...
static char _abort_indicator;

static wget_vector_t * http_proxies, *https_proxies;

int
wget_http_isseperator (char c)
{
...
int
wget_http_istoken (char c)
{
  return c > 32 && c <= 126 && !_http_isseperator (c);
}

const char *
wget_http_parse_token (const char *s, const char **token)
{
  const char *p;
...
const char *
wget_http_parse_quoted_string (const char *s, const char **qstring)
{
  if (*s == '\"')
    {
      const char *p = ++s;
...
const char *
wget_http_parse_param (const char *s, const char **param, const char **value)
{
  const char *p;
...
const char *
wget_http_parse_name (const char *s, const char **name)
{
  while (isblank (*s))
    s++;
...
const char *
wget_parse_name_fixed (const char *s, const char **name, size_t * namelen)
{
  while (isblank (*s))
    s++;
...
compare_param (wget_http_header_param_t * p1, wget_http_header_param_t * p2)
{
...
*/
const char *
wget_http_parse_link (const char *s, wget_http_link_t * link)
{
...
const char *
wget_http_parse_digest (const char *s, wget_http_digest_t * digest)
{
  const char *p;
...
const char *
wget_http_parse_challenge (const char *s, wget_http_challenge_t * challenge)
{
  const char *old;
      ...
      if (param.name)
        {
          ...
          wget_stringmap_put_noalloc (challenge->params, param.name,
                                      param.value);
        }
...
const char *
wget_http_parse_challenges (const char *s, wget_vector_t * challenges)
{
...
const char *
wget_http_parse_location (const char *s, const char **location)
{
  const char *p;
...
const char *
wget_http_parse_transfer_encoding (const char *s, char *transfer_encoding)
{
  while (isblank (*s))
    s++;
...
const char *
wget_http_parse_content_type (const char *s, const char **content_type,
                              const char **charset)
{
  wget_http_header_param_t param;
  const char *p;
...
const char *
wget_http_parse_content_disposition (const char *s, const char **filename)
{
  wget_http_header_param_t param;
  char *p;
              ...
              if (!*filename)
                {
                  ...
                  if (!wget_str_is_valid_utf8 (p))
                   ...
                   && !wget_strcasecmp_ascii ("filename*", param.name))
            {
              // RFC5987
...
const char *
wget_http_parse_strict_transport_security (const char *s, time_t * maxage,
...
const char *
wget_http_parse_content_encoding (const char *s, char *content_encoding)
{
  while (isblank (*s))
    s++;

  if (!wget_strcasecmp_ascii (s, "gzip")
      || !wget_strcasecmp_ascii (s, "x-gzip"))
  ...
  else if (!wget_strcasecmp_ascii (s, "xz")
           || !wget_strcasecmp_ascii (s, "lzma")
           || !wget_strcasecmp_ascii (s, "x-lzma"))
...
const char *
wget_http_parse_connection (const char *s, char *keep_alive)
{
  while (isblank (*s))
    s++;
...
const char *
wget_http_parse_etag (const char *s, const char **etag)
{
  const char *p;
...
wget_http_parse_full_date (const char *s)
{
  ...
  if (sscanf
      (s, " %*[a-zA-Z], %02d %3s %4d %2d:%2d:%2d", &day, mname, &year, &hour,
       &min, &sec) >= 6)
    {
      // RFC 822 / 1123: Wed, 09 Jun 2021 10:18:14 GMT
    }
    ...
    if (sscanf
        (s, " %*[a-zA-Z], %2d-%3s-%4d %2d:%2d:%2d", &day, mname, &year, &hour,
         &min, &sec) >= 6)
      {
        // RFC 850 / 1036 or Netscape: Wednesday, 09-Jun-21 10:18:14 or Wed, 09-Jun-2021 10:18:14
      }
      ...
      if (sscanf
          (s, " %*[a-zA-Z] %3s %2d %2d:%2d:%2d %4d", mname, &day, &hour, &min,
           &sec, &year) >= 6)
        {
          // ANSI C's asctime(): Wed Jun 09 10:18:14 2021
        }
  ...
  if (mon < 1 || mon > 12 || day < 1
      || (day > days_per_month[mon - 1] + leap_month) || hour < 0 || hour > 23
...
char *
wget_http_print_date (time_t t, char *buf, size_t bufsize)
{
  ...
  if (gmtime_r (&t, &tm))
    {
      snprintf (buf, bufsize, "%s, %02d %s %d %02d:%02d:%02d GMT",
                dnames[tm.tm_wday], tm.tm_mday, mnames[tm.tm_mon],
...
*/
const char *
wget_http_parse_setcookie (const char *s, wget_cookie_t * cookie)
{
  const char *name, *p;
...
wget_http_parse_response_header (char *buf)
{
  const char *s;
  char *line, *eol;
  const char *name;
      ...
      switch (*name | 0x20)
        ...
        case 'c':
          if (!strncasecmp (name, "Content-Encoding", namelen))
            ...
            {
              wget_http_parse_content_type (s, &resp->content_type,
            ...
            {
              wget_http_parse_content_disposition (s,
                                                   &resp->content_filename);
            }
          ...
          if (!strncasecmp (name, "Last-Modified", namelen))
            {
              ...
              resp->last_modified = wget_http_parse_full_date (s);
                   ...
                   && !strncasecmp (name, "Location", namelen))
            {
              xfree (resp->location);
              wget_http_parse_location (s, &resp->location);
            }
          else if (resp->code / 100 == 3
                   && !strncasecmp (name, "Link", namelen))
            {
              // debug_printf("s=%.31s\n",s);
              ...
              if (!resp->links)
                {
                  ...
                  wget_vector_set_destructor (resp->links,
                                              (void (*)(void *))
                                              wget_http_free_link);
                }
        ...
        case 's':
          if (!strncasecmp (name, "Set-Cookie", namelen))
                  ...
                  if (!resp->cookies)
                    {
                      ...
                      wget_vector_set_destructor (resp->cookies,
                                                  (void (*)(void *))
                                                  wget_cookie_deinit);
                    }
            ...
            {
              resp->hsts = 1;
              wget_http_parse_strict_transport_security (s,
                                                         &resp->hsts_maxage,
                                                         &resp->
              ...
              if (!resp->challenges)
                {
                  ...
                  wget_vector_set_destructor (resp->challenges,
                                              (void (*)(void *))
              ...
              if (!resp->digests)
                {
                  ...
                  wget_vector_set_destructor (resp->digests,
                                              (void (*)(void *))
...
wget_http_request_t *
wget_http_create_request (const wget_iri_t * iri, const char *method)
{
  wget_http_request_t *req = xcalloc (1, sizeof (wget_http_request_t));

  wget_buffer_init (&req->esc_resource, req->esc_resource_buf,
                    sizeof (req->esc_resource_buf));
  wget_buffer_init (&req->esc_host, req->esc_host_buf,
                    sizeof (req->esc_host_buf));
  ...
  wget_vector_set_destructor (req->headers,
                              (void (*)(void *)) wget_http_free_param);

  return req;
}

void
wget_http_add_header_vprintf (wget_http_request_t * req, const char *name,
...
void
wget_http_add_header_printf (wget_http_request_t * req, const char *name,
                             const char *fmt, ...)
{
  va_list args;
...
void
wget_http_add_header (wget_http_request_t * req, const char *name,
                      const char *value)
{
  wget_http_header_param_t param = {
    .name = strdup (name),
...
void
wget_http_add_header_param (wget_http_request_t * req,
                            wget_http_header_param_t * param)
{
...
void
wget_http_add_credentials (wget_http_request_t * req,
                           wget_http_challenge_t * challenge,
                           const char *username, const char *password)
{
  ...
  if (!wget_strcasecmp_ascii (challenge->auth_scheme, "basic"))
    {
      const char *encoded =
        wget_base64_encode_printf_alloc ("%s:%s", username, password);
...
# define MD5_DIGEST_LENGTH 16
      ...
      // A2BUF = H(method ":" path)
      wget_md5_printf_hex (a2buf, "%s:/%s", req->method,
      ...
      if (!wget_strcmp (qop, "auth") || !wget_strcmp (qop, "auth-int"))
        {
          ...
          wget_md5_printf_hex (response_digest, "%s:%s:00000001:%s:%s:%s",
                               a1buf, nonce, /* nc, */ cnonce, qop, a2buf);
        }
      else
        {
          ...
          wget_md5_printf_hex (response_digest, "%s:%s:%s", a1buf, nonce,
                               a2buf);
        }
      ...
      wget_buffer_printf2 (&buf,
                           ...
                           username, realm, nonce, req->esc_resource.data,
                           response_digest);

      if (!wget_strcmp (qop, "auth"))
        wget_buffer_printf_append2 (&buf,
...
#ifdef WITH_LIBNGHTTP2
static ssize_t
_send_callback (nghttp2_session * session G_GNUC_WGET_UNUSED,
                const uint8_t * data, size_t length,
                int flags G_GNUC_WGET_UNUSED, void *user_data)
{
...
static ssize_t
_recv_callback (nghttp2_session * session G_GNUC_WGET_UNUSED,
                uint8_t * buf, size_t length, int flags G_GNUC_WGET_UNUSED,
                void *user_data)
{
  wget_http_connection_t *conn = (wget_http_connection_t *) user_data;
  int rc;
...
static void
_print_frame_type (int type, const char tag)
{
...
static int
_on_frame_send_callback (nghttp2_session * session G_GNUC_WGET_UNUSED,
                         const nghttp2_frame * frame,
                         void *user_data G_GNUC_WGET_UNUSED)
{
  ...
  if (frame->hd.type == NGHTTP2_HEADERS)
    {
      ...
      for (unsigned i = 0; i < frame->headers.nvlen; i++)
        debug_printf ("[FRAME] > %.*s: %.*s\n", (int) nva[i].namelen,
                      nva[i].name, (int) nva[i].valuelen, nva[i].value);
    }
...
static int
_on_frame_recv_callback (nghttp2_session * session G_GNUC_WGET_UNUSED,
                         const nghttp2_frame * frame,
                         void *user_data G_GNUC_WGET_UNUSED)
{
  _print_frame_type (frame->hd.type, '<');
...
static int
_on_header_callback (nghttp2_session * session G_GNUC_WGET_UNUSED,
                     const nghttp2_frame * frame, const uint8_t * name,
                     size_t namelen, const uint8_t * value, size_t valuelen,
                     uint8_t flags G_GNUC_WGET_UNUSED,
                     void *user_data G_GNUC_WGET_UNUSED)
{
          ...
          if (frame->headers.cat == NGHTTP2_HCAT_RESPONSE)
            {
              ...
              switch (namelen)
                ...
                case 4:
                  ...
                  else if (!memcmp (name, "link", namelen)
                           && resp->code / 100 == 3)
                    {
                      // debug_printf("s=%.31s\n",s);
                      wget_http_link_t link;
                  ...
                  if (!memcmp (name, "digest", namelen))
                    {
                      ...
                      wget_vector_add (resp->digests, &digest,
                                       sizeof (digest));
                    }
                  break;
                case 7:
                  if (!memcmp (name, ":status", namelen) && valuelen == 3)
                    {
                      ctx->resp->code =
                        ((value[0] - '0') * 10 + (value[1] - '0')) * 10 +
                        (value[2] - '0');
                    }
                  break;
                case 8:
                  if (resp->code / 100 == 3
                      && !memcmp (name, "location", namelen))
                    {
                      xfree (resp->location);
                      wget_http_parse_location (s, &resp->location);
                    }
                ...
                case 16:
                  if (!memcmp (name, "content-encoding", namelen))
                    {
                      wget_http_parse_content_encoding (s,
                                                        &resp->
                                                        content_encoding);
                    }
                ...
                case 17:
                  if (!memcmp (name, "transfer-encoding", namelen))
                    {
                      wget_http_parse_transfer_encoding (s,
                                                         &resp->
                                                         transfer_encoding);
                    }
                  break;
 ...
 */
static int
_on_stream_close_callback (nghttp2_session * session, int32_t stream_id,
                           uint32_t error_code G_GNUC_WGET_UNUSED,
                           void *user_data G_GNUC_WGET_UNUSED)
{
  wget_http_request_t *req =
 ...
 */
static int
_on_data_chunk_recv_callback (nghttp2_session * session,
                              uint8_t flags G_GNUC_WGET_UNUSED,
                              int32_t stream_id, const uint8_t * data,
                              size_t len, void *user_data G_GNUC_WGET_UNUSED)
{
  wget_http_request_t *req =
    nghttp2_session_get_stream_user_data (session, stream_id);
...
setup_nghttp2_callbacks (nghttp2_session_callbacks * callbacks)
{
  ...
  nghttp2_session_callbacks_set_on_frame_send_callback (callbacks,
                                                        _on_frame_send_callback);
  nghttp2_session_callbacks_set_on_frame_recv_callback (callbacks,
                                                        _on_frame_recv_callback);
  nghttp2_session_callbacks_set_on_stream_close_callback (callbacks,
                                                          _on_stream_close_callback);
  nghttp2_session_callbacks_set_on_data_chunk_recv_callback (callbacks,
                                                             _on_data_chunk_recv_callback);
...
wget_http_open (wget_http_connection_t ** _conn, const wget_iri_t * iri)
{
  ...
  int rc, ssl = iri->scheme == WGET_IRI_SCHEME_HTTPS;
  ...
  if (iri->scheme == WGET_IRI_SCHEME_HTTP && http_proxies)
    {
      ...
      proxy =
        wget_vector_get (http_proxies,
                         (++next_http_proxy) %
                         wget_vector_size (http_proxies));
  ...
  else if (iri->scheme == WGET_IRI_SCHEME_HTTPS && https_proxies)
    {
      wget_thread_mutex_lock (&mutex);
      proxy =
        wget_vector_get (https_proxies,
                         (++next_https_proxy) %
                         wget_vector_size (https_proxies));
      wget_thread_mutex_unlock (&mutex);
...
#ifdef WITH_LIBNGHTTP2
      if ((conn->protocol =
           wget_tcp_get_protocol (conn->tcp)) == WGET_PROTOCOL_HTTP_2_0)
        {
          ...
          rc =
            nghttp2_session_client_new (&conn->http2_session, callbacks,
                                        conn);
          ...
          if ((rc =
               nghttp2_submit_settings (conn->http2_session,
                                        NGHTTP2_FLAG_NONE, NULL, 0)))
            {
              error_printf (_
...
void
wget_http_close (wget_http_connection_t ** conn)
{
  if (*conn)
    {
      ...
      if ((*conn)->http2_session)
        {
          int rc =
            nghttp2_session_terminate_session ((*conn)->http2_session,
                                               NGHTTP2_NO_ERROR);
          if (rc)
...
G_GNUC_WGET_NONNULL ((1, 2))
  _http_send_request (wget_http_connection_t * conn, wget_http_request_t * req,
  ...
  if (wget_tcp_get_protocol (conn->tcp) == WGET_PROTOCOL_HTTP_2_0)
    {
      ...
      resource[0] = '/';
      memcpy (resource + 1, req->esc_resource.data,
              req->esc_resource.length + 1);
      INIT_NV (&nvs[0], ":method", "GET") INIT_NV_CS (&nvs[1], ":path",
                                                      resource)
        INIT_NV (&nvs[2], ":scheme", "https") INIT_NV_CS (&nvs[3],
                                                          ":authority",
                                                          req->esc_host.
                                                          data) nvp = &nvs[4];
...
int
wget_http_send_request (wget_http_connection_t * conn,
                        wget_http_request_t * req)
{
...
int
wget_http_send_request_with_body (wget_http_connection_t * conn,
                                  wget_http_request_t * req, const void *body,
                                  size_t length)
{
...
wget_http_request_to_buffer (wget_http_request_t * req, wget_buffer_t * buf)
{
  ...
  if (req->scheme == WGET_IRI_SCHEME_HTTP
      && wget_vector_size (http_proxies) > 0)
    {
      use_proxy = 1;
  ...
  else if (req->scheme == WGET_IRI_SCHEME_HTTPS
           && wget_vector_size (https_proxies) > 0)
    {
      use_proxy = 1;
      wget_buffer_strcat (buf, req->scheme);
      wget_buffer_memcat (buf, "://", 3);
...
wget_http_response_t *
wget_http_get_response_cb (wget_http_connection_t * conn, wget_http_request_t * req, unsigned int flags, int (*header_callback) (void *context, wget_http_response_t * resp), int (*body_callback) (void *context, const char *data, size_t length), void *context)  // given to body_callback and header_callback
{
  ...
  if (conn->protocol == WGET_PROTOCOL_HTTP_2_0)
    {
      ...
      struct _body_callback_context ctx = {.resp = resp,.context =
                                           context,.body_callback = body_callback };
      req->nghttp2_context = &ctx;

      int timeout = wget_tcp_get_timeout (conn->tcp);

      for (int rc = 0;
           rc == 0 && !ctx.done && !conn->abort_indicator
             && !_abort_indicator;)
        {
          ioflags = 0;
  ...
  while ((nbytes =
          wget_tcp_read (conn->tcp, buf + nread, bufsize - nread)) > 0)
    {
  ...
  if (!resp || resp->code / 100 == 1 || resp->code == 204 || resp->code == 304
      || (resp->transfer_encoding == transfer_encoding_identity
          && resp->content_length == 0 && resp->content_length_valid))
    {
  ...
  if (resp->transfer_encoding != transfer_encoding_identity)
          ...
          while ((!(end = strchr (p, '\r')) || end[1] != '\n'))
            {
              ...
              if ((nbytes =
                   wget_tcp_read (conn->tcp, buf + body_len,
                                  bufsize - body_len)) <= 0)
                goto cleanup;
  ...
  else
    {
      ...
      while (!conn->abort_indicator && !_abort_indicator
             && (nbytes = wget_tcp_read (conn->tcp, buf, bufsize)) > 0)
        {
          body_len += nbytes;
...
static int
_get_body (void *userdata, const char *data, size_t length)
{
...
wget_http_response_t *
wget_http_get_response (wget_http_connection_t * conn,
                        int (*header_callback) (void *context,
                                                wget_http_response_t * resp),
                        wget_http_request_t * req, unsigned int flags)
{
  wget_http_response_t *resp;
  wget_buffer_t *body = wget_buffer_alloc (102400);

  resp =
    wget_http_get_response_cb (conn, req, flags, header_callback, _get_body,
                               body);
...
static int
_get_fd (void *context, const char *data, size_t length)
{
  int fd = *(int *) context;
...
wget_http_response_t *
wget_http_get_response_fd (wget_http_connection_t * conn,
                           int (*header_callback) (void *,
                                                   wget_http_response_t *),
                           int fd, unsigned int flags)
{
  wget_http_response_t *resp =
    wget_http_get_response_cb (conn, NULL, flags, header_callback, _get_fd,
                               &fd);
...
static int
_get_stream (void *context, const char *data, size_t length)
{
...
{
  wget_http_response_t *resp =
    wget_http_get_response_cb (conn, NULL, flags, header_callback,
                               _get_stream, stream);
...
wget_http_response_t *
wget_http_get_response_func (wget_http_connection_t * conn,
                             int (*header_callback) (void *,
                                                     wget_http_response_t *),
                             int (*body_callback) (void *, const char *,
                                                   size_t), void *context,
                             unsigned int flags)
{
  wget_http_response_t *resp =
    wget_http_get_response_cb (conn, NULL, flags, header_callback,
                               body_callback, context);
...
_parse_proxies (const char *proxy, const char *encoding)
{
  if (proxy)
    {
      ...
      wget_vector_set_destructor (proxies,
                                  (void (*)(void *)) wget_iri_free_content);
          ...
          if (p != s)
            {
              ...
              memcpy (host, s, p - s);
              host[p - s] = 0;
              wget_vector_add_noalloc (proxies,
                                       wget_iri_parse (host, encoding));
            }
        }
      if (*s)
