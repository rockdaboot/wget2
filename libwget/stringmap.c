...
static unsigned int G_GNUC_WGET_PURE
hash_string_nocase (const char *key)
{
  unsigned int hash = 0;  // use 0 as SALT if hash table attacks doesn't matter
...
{
  return wget_hashmap_create (max, -2,
                              (unsigned int (*)(const void *)) hash_string,
...
wget_stringmap_browse (const wget_stringmap_t * h,
                       int (*browse) (void *ctx, const char *key,
                                      void *value), void *ctx)
{
  return wget_hashmap_browse (h,
                              (int (*)(void *, const void *, void *)) browse,
                              ctx);
}
