...
struct _wget_hashmap_st
{
  ...
  void (*key_destructor) (void *);  // key destructor function
  void (*value_destructor) (void *);  // value destructor function
...
hashmap_find_entry (const wget_hashmap_t * h, const char *key,
...
hashmap_rehash (wget_hashmap_t * h, int newmax, int recalc_hash)
{
...
G_GNUC_WGET_NONNULL ((1, 3))
  hashmap_new_entry (wget_hashmap_t * h, unsigned int hash, const char *key,
                     const char *value)
{
  ENTRY *entry;
  int pos = hash % h->max;
...
hashmap_remove_entry (wget_hashmap_t * h, const char *key, int free_kv)
{
  ...
  for (entry = h->entry[pos]; entry; prev = entry, entry = next)
    {
      ...
      if (hash == entry->hash
          && (key == entry->key || !h->cmp (key, entry->key)))
        {
          if (prev)
            prev->next = next;
          else
...
wget_hashmap_browse (const wget_hashmap_t * h,
                     int (*browse) (void *ctx, const void *key, void *value),
                     void *ctx)
{
  if (h)
    {
