...
FILE *
wget_vpopenf (const char *type, const char *fmt, va_list args)
{
  FILE *fp = NULL;
  char sbuf[1024];
  wget_buffer_t buf;
...
FILE *
wget_popenf (const char *type, const char *fmt, ...)
{
  FILE *fp;
  va_list args;
             ...
             const char *const *argv)
{
  ...
  if ((pid =
       wget_fd_popen3 (fpin ? &fdin : NULL, fpout ? &fdout : NULL,
                       fperr ? (fperr != fpout ? &fderr : &fdout) : NULL,
                       argv)) > 0)
    {
      if (fpin)
        *fpin = fdopen (fdin, "w");
      ...
      if (fperr && fperr != fpout)
        *fperr = fdopen (fderr, "r");
    }
