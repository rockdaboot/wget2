...
wget_bar_update (const wget_bar_t * bar, int slotpos, int max, int cur)
{
  ...
  if (slot->cols != cols || (int) (slot->ratio * 100) != (int) (ratio * 100)
      || slot->first)
    {
      slot->cols = cols;
      slot->ratio = ratio;
      ...
      printf ("%3d%% [%.*s>%.*s]", (int) (ratio * 100), cols - 1, bar->filled,
...
{
  printf ("\033[s\033[%dA\033[6G[%-*.*s]\033[u", bar->nslots - slotpos,
          bar->max_width, bar->max_width, s);
  fflush (stdout);
}
