...
typedef struct
{
  WGET_HTML_PARSED_RESULT result;
  wget_vector_t *additional_tags;
  wget_vector_t *ignore_tags;
  char found_robots, found_content_type;
} _html_context_t;
...
static void
_html_get_url (void *context, int flags, const char *tag, const char *attr,
               const char *val, size_t len, size_t pos G_GNUC_WGET_UNUSED)
{
  ...
  if ((flags & XML_FLG_BEGIN) && (*tag | 0x20) == 'm'
      && !wget_strcasecmp_ascii (tag, "meta"))
    {
  ...
  if ((flags & XML_FLG_ATTRIBUTE) && val)
    {
      ...
      if ((*tag | 0x20) == 'm' && !wget_strcasecmp_ascii (tag, "meta"))
        {
          if (!ctx->found_robots)
            {
              if (!wget_strcasecmp_ascii (attr, "name")
                  && !wget_strncasecmp_ascii (val, "robots", len))
                {
                  ctx->found_robots = 1;
                  return;
                }
            }
          else if (ctx->found_robots
                   && !wget_strcasecmp_ascii (attr, "content"))
            {
              char *p;
              ...
              while (*value)
                {
                  ...
                  if (!strncasecmp (value, "all", p - value)
                      || !strncasecmp (value, "follow", p - value))
                    res->follow = 1;
                  else if (!strncasecmp (value, "nofollow", p - value)
                           || !strncasecmp (value, "none", p - value))
                    res->follow = 0;
          ...
          if (ctx->found_content_type && !res->encoding)
            ...
            {
              if (!wget_strcasecmp_ascii (attr, "http-equiv")
                  && !wget_strncasecmp_ascii (val, "Content-Type", len))
                {
      ...
      if (ctx->ignore_tags)
        {
          if (wget_vector_find (ctx->ignore_tags, &(wget_html_tag_t)
                                {
                                  .name = tag,.attribute = NULL}
                                ) != -1
              || wget_vector_find (ctx->ignore_tags, &(wget_html_tag_t)
                                   {
                                     .name = tag,.attribute = attr}
                                   ) != -1)
            return;
        }
      ...
      if (maybe[(unsigned char) *attr | 0x20] && attr[1] && attr[2])
        found =
          bsearch (attr, attrs, countof (attrs), sizeof (attrs[0]),
                   (int (*)(const void *, const void *))
                   wget_strcasecmp_ascii) != NULL;

      // search the dynamic list for a tag/attr match
      if (!found && ctx->additional_tags)
        {
          if (wget_vector_find (ctx->additional_tags, &(wget_html_tag_t)
                                {
                                  .name = tag,.attribute = NULL}
                                ) != -1
              || wget_vector_find (ctx->additional_tags, &(wget_html_tag_t)
                                   {
                                     .name = tag,.attribute = attr}
                                   ) != -1)
            found = 1;
...
wget_html_get_urls_inline (const char *html, wget_vector_t * additional_tags,
                           wget_vector_t * ignore_tags)
{
  ...
  wget_html_parse_buffer (html, _html_get_url, &context,
                          HTML_HINT_REMOVE_EMPTY_CONTENT);
