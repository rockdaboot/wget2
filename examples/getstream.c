...
static void
header_callback (void *context G_GNUC_WGET_UNUSED,
...
static void
stream_callback (void *context G_GNUC_WGET_UNUSED, const char *data,
                 size_t len)
{
...
main (int argc G_GNUC_WGET_UNUSED, const char *const *argv G_GNUC_WGET_UNUSED)
{
  ...
  wget_global_init (
                    ...
                    WGET_INFO_STREAM, stderr, NULL);

  // get and parse the m3u playlist file
  resp =
    wget_http_get (WGET_HTTP_URL, "http://listen.radionomy.com/gothica.m3u",
                   NULL);

  if (resp && resp->code == 200
      && !wget_strcasecmp_ascii (resp->content_type, "audio/x-mpegurl"))
    {
  ...
  if (stream_url)
    {
      resp = wget_http_get (WGET_HTTP_URL, stream_url, WGET_HTTP_HEADER_ADD, "Icy-Metadata", "1",  // we want in-stream title/actor information
