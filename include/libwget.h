...
char *
	wget_charset_transcode(const char *src, const char *src_encoding, const char *dst_encoding) G_GNUC_WGET_MALLOC;
int
...
#ifndef HAVE_STRNDUP
char *
	strndup(const char *s, size_t n) G_GNUC_WGET_MALLOC G_GNUC_WGET_NONNULL_ALL;
#endif
...
#ifndef HAVE_DPRINTF
int
	vdprintf(int fd, const char *restrict fmt, va_list) G_GNUC_WGET_PRINTF_FORMAT(2,0);
int
	dprintf(int fd, const char *restrict fmt, ...) G_GNUC_WGET_PRINTF_FORMAT(2,3);
#endif /* HAVE_DPRINTF */
...
void *
	wget_memdup(const void *s, size_t n) G_GNUC_WGET_ALLOC_SIZE(2);
char *
	wget_strdup(const char *s) G_GNUC_WGET_MALLOC;
char *
	wget_strmemdup(const void *s, size_t n) G_GNUC_WGET_ALLOC_SIZE(2);
void
...
size_t
	wget_buffer_vprintf_append2(wget_buffer_t *buf, const char *fmt, va_list args) G_GNUC_WGET_NONNULL((1,2)) G_GNUC_WGET_PRINTF_FORMAT(2,0);
size_t
	wget_buffer_printf_append2(wget_buffer_t *buf, const char *fmt, ...) G_GNUC_WGET_NONNULL((1,2)) G_GNUC_WGET_PRINTF_FORMAT(2,3);
size_t
	wget_buffer_vprintf2(wget_buffer_t *buf, const char *fmt, va_list args) G_GNUC_WGET_NONNULL((1,2)) G_GNUC_WGET_PRINTF_FORMAT(2,0);
size_t
	wget_buffer_printf2(wget_buffer_t *buf, const char *fmt, ...) G_GNUC_WGET_NONNULL((1,2)) G_GNUC_WGET_PRINTF_FORMAT(2,3);
...
int
	wget_iri_isunreserved(char c) G_GNUC_WGET_PURE;
int
...
int
	wget_tcp_get_protocol(wget_tcp_t *tcp);
int
...
wget_http_response_t *
	wget_http_get_response_cb(wget_http_connection_t *conn, wget_http_request_t *req, unsigned int flags,
		int (*header_callback)(void *context, wget_http_response_t *),
		int (*body_callback)(void *context, const char *data, size_t length),
		void *context) G_GNUC_WGET_NONNULL((1,5));
//HTTP_RESPONSE *
...
wget_http_response_t *
	wget_http_get_response(wget_http_connection_t *conn,
		...
		wget_http_request_t *req, unsigned int flags) G_GNUC_WGET_NONNULL((1));
wget_http_response_t *
	wget_http_get_response_fd(wget_http_connection_t *conn,
		int(*header_callback)(void *, wget_http_response_t *),
		int fd, unsigned int flags) G_GNUC_WGET_NONNULL_ALL;
wget_http_response_t *
	wget_http_get_response_stream(wget_http_connection_t *conn,
		int(*header_callback)(void *, wget_http_response_t *),
		FILE *stream, unsigned int flags) G_GNUC_WGET_NONNULL_ALL;
wget_http_response_t *
	wget_http_get_response_func(wget_http_connection_t *conn,
		int(*header_callback)(void *, wget_http_response_t *),
		int(*body_callback)(void *, const char *, size_t), void *context, unsigned int flags) G_GNUC_WGET_NONNULL((1,2));
...
int
	wget_http_send_request_with_body(wget_http_connection_t *conn, wget_http_request_t *req, const void *body, size_t length) G_GNUC_WGET_NONNULL((1,2));
ssize_t
...
/**
 * wget_hash_fast:
 * @algorithm: one of the hashing algorithms supported by libwget, returned by wget_hash_get_algorithm().
 ...
 *
 * The caller must ensure that the provided output buffer @digest is large enough
 ...
 * different lengths of data. To get the output length of the chosen algorithm @algorithm,
 * call wget_hash_get_len().
 *
 * NOTE: This function's behavior depends on the underlying cryptographic engine libwget was compiled with.
 *
 ...
 * @algorithm: one of the hashing algorithms supported by libwget, returned by wget_hash_get_algorithm().
 *
 * Return the output length of the given hashing algorithm.
 *
 ...
 * NOTE: This function's behavior depends on the underlying cryptographic engine libwget was compiled with.
 *
 * Returns the length of the output data generated by the algorithm.
 */
int
	wget_hash_get_len(wget_digest_algorithm_t algorithm);
/**
 * wget_hash_init:
 * @dig: caller-provided pointer to a @wget_hash_hd_t structure where the handle to this hashing primitive will be stored,
 * needed in subsequent calls to wget_hash().
 * @algorithm: one of the hashing algorithms supported by libwget, returned by wget_hash_get_algorithm().
 *
 * Initialize the cryptographic engine to compute hashes
 * with the given hashing algorithm, as well as the hashing algorithm itself.
...
/**
 * wget_hash:
 * @handle: handle to the hashing primitive returned by a subsequent call to wget_hash_init().
 * @text: input data.
 * @textlen: length of the input data.
 *
 ...
 * buffer @digest. The caller must ensure that the provided output buffer @digest
 * is large enough to store the hash. To get the output length of the chosen algorithm
 * @algorithm, call wget_hash_get_len().
 *
 * NOTE: This function's behavior depends on the underlying cryptographic engine libwget was compiled with.
 */
void
...
int
	wget_hash_file_fd(const char *type, int fd, char *digest_hex, size_t digest_hex_size, off_t offset, off_t length) G_GNUC_WGET_NONNULL_ALL;
int
	wget_hash_file_offset(const char *type, const char *fname, char *digest_hex, size_t digest_hex_size, off_t offset, off_t length) G_GNUC_WGET_NONNULL_ALL;
int
	wget_hash_file(const char *type, const char *fname, char *digest_hex, size_t digest_hex_size) G_GNUC_WGET_NONNULL_ALL;
...
wget_metalink_t
	*metalink3_parse(const char *xml) G_GNUC_WGET_NONNULL((1)),
	*metalink4_parse(const char *xml) G_GNUC_WGET_NONNULL((1));
void
...
wget_bar_t *
	wget_bar_init(wget_bar_t *bar, int nslots, int max_width);
void
	wget_bar_deinit(wget_bar_t *bar);
void
	wget_bar_free(wget_bar_t **bar);
void
	wget_bar_update(const wget_bar_t *bar, int slotpos, int max, int cur);
void
	wget_bar_print(wget_bar_t *bar, int slotpos, const char *s);
ssize_t
