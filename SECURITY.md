...
current maintainers is always available via the AUTHORS file in this repository. In
order to send an encrypted email, please use the keyring available at the following URL:
...
## LLM-Generated Security Reports

With the rise of AI-assisted vulnerability research, we receive an increasing
number of reports generated by Large Language Models (LLMs). While we appreciate
all security contributions, please note:

### Commodity Findings

Findings that can be trivially generated by anyone using public LLMs without
deep domain knowledge (e.g., generic "potential buffer overflow" patterns,
standard CWE listings without project-specific context, or automated SAST-style
output) are **not treated as confidential**. These may be disclosed or discussed
publicly at any time by the project maintainers.

### Quality Expectations

For LLM-assisted reports to receive confidential handling and coordinated
disclosure:

- The report must demonstrate **project-specific understanding** (code paths,
  configuration, threat model)
- Include **verified reproduction steps** on a supported version
- Show **impact analysis** beyond generic CWE descriptions
- False positives and unverified theoretical issues will be closed without
  coordinated disclosure

### Reporting Channel

LLM-generated reports should still be submitted via our confidential issue
tracker, but maintainers reserve the right to reclassify commodity findings as
non-confidential immediately upon triage.
