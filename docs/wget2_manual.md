...
* -i, --input-file=file
  ...
  input file, those on the command lines will be the first ones to be retrieved. If --force-html is not specified, then file
...
* --metalink-over-http
  ...
  download.  If no valid Metalink metadata is found, it falls back to ordinary HTTP download.
...
* -B, --base=URL
  ...
  -i/--input-file option (together with --force-html, or when the input file was fetched remotely from a server describing it as
  HTML). This is equivalent to the presence of a "BASE" tag in the HTML input file, with URL as the value for the "href"
  attribute.
...
* -U agent-string, --user-agent=agent-string
  ...
  Specifying empty user agent with --user-agent="" instructs Wget not to send the "User-Agent" header in HTTP
  requests.
...
* --hsts-file=file
  ...
  If you supply your own HSTS database via --hsts-file, be aware that Wget may modify the provided file if any
