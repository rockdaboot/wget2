...
http_server_tid, https_server_tid, ftp_server_tid, ftps_server_tid;
static int
http_server_port,
  https_server_port,
  ftp_server_port, ftps_server_port, ftps_implicit, terminate, keep_tmpfiles;
/*static const char
 *response_code = "200 Dontcare",
 *response_body = "";
 static WGET_VECTOR
 *response_headers; */
static wget_vector_t * request_urls;
static wget_test_url_t * urls;
static size_t nurls;
static wget_test_ftp_io_t * ios;
static size_t nios;
static int ios_ordered;
static char tmpdir[128];
static const char *server_hello;

static void
sigterm_handler (int sig G_GNUC_WGET_UNUSED)
{
  terminate = 1;
}

static void *
_http_server_thread (void *ctx)
{
  ...
  sigaction (SIGTERM, &(struct sigaction)
             {
               .sa_handler = sigterm_handler}, NULL);
#endif
          ...
          if ((nbytes = wget_tcp_read (tcp, buf, sizeof (buf) - 1)) > 0)
            {
              ...
              for (p = strstr (buf, "\r\n");
                   sscanf (p, "\r\n%63[^:]: %255[^\r]", tag, value) == 2;
                   p = strstr (p + 2, "\r\n"))
                {
                  if (!wget_strcasecmp_ascii (tag, "Range"))
                    {
                      if ((byterange =
                           sscanf (value, "bytes=%zd-%zd", &from_bytes,
                                   &to_bytes)) < 1)
                        byterange = 0;
                    }
                  else if (url
                           && !wget_strcasecmp_ascii (tag, "Authorization"))
                    {
                      ...
                      if (!wget_strcasecmp_ascii (auth_scheme, "basic"))
                        {
                          const char *encoded =
                            wget_base64_encode_printf_alloc ("%s:%s",
                                                             url->
                                                             auth_username,
                                                             url->
                                                             auth_password);
                  ...
                  for (it = 0; it < nurls; it++)
                    {
                      if (!strcmp (request_url, urls[it].name) ||
                          (!strncmp
                           (request_url, urls[it].name, request_url_length)
                           && !strcmp (urls[it].name + request_url_length,
                                       "index.html")))
                        {
                          url = &urls[it];
                          break;
              ...
              if (byterange)
                {
                  if (from_bytes > to_bytes
                      || from_bytes >= (int) strlen (url->body))
                    {
                      wget_tcp_printf (tcp,
                  ...
                  nbytes = snprintf (buf, sizeof (buf),
                                     ...
                                     body_len, from_bytes, to_bytes,
                                     body_len);
                  for (it = 0;
                       it < countof (url->headers) && url->headers[it]; it++)
                    {
                      nbytes +=
                        snprintf (buf + nbytes, sizeof (buf) - nbytes,
                                  "%s\r\n", url->headers[it]);
                    }
                  nbytes +=
                    snprintf (buf + nbytes, sizeof (buf) - nbytes, "\r\n");
                  if (!strcmp (method, "GET") || !strcmp (method, "POST"))
                    nbytes +=
                      snprintf (buf + nbytes, sizeof (buf) - nbytes, "%.*s",
                                (int) body_len, url->body + from_bytes);
                }
              else
                {
                  // create response
...
static void *
_ftp_server_thread (void *ctx)
{
  wget_tcp_t *tcp = NULL, *parent_tcp = ctx, *pasv_parent_tcp =
    NULL, *pasv_tcp = NULL;
  char buf[4096];
  ssize_t nbytes;
  int pasv_port, found;
          ...
          while ((nbytes = wget_tcp_read (tcp, buf, sizeof (buf) - 1)) > 0)
            {
              ...
              if (!strncmp (buf, "PASV", 4) || !strncmp (buf, "EPSV", 4))
                {
                  ...
                  if (!strncmp (buf, "EPSV", 4))
                    {
                      switch (atoi (buf + 4))
                        {
                        case 1:
                          wget_tcp_set_family (pasv_parent_tcp,
                  ...
                  if (wget_tcp_listen (pasv_parent_tcp, "localhost", NULL, 5)
                      != 0)
                    {
              ...
              if (ios[io_pos].send_url && pasv_tcp)
                {
                  // send data
                  wget_tcp_printf (pasv_tcp, "%s",
                                   ios[io_pos].send_url->body);
                  wget_tcp_deinit (&pasv_tcp);
...
static void
_empty_directory (const char *dirname)
{
  DIR *dir;
  struct dirent *dp;
...
void
wget_test_start_server (int first_key, ...)
{
  static wget_tcp_t *http_parent_tcp, *https_parent_tcp, *ftp_parent_tcp,
    *ftps_parent_tcp;
  int rc, key;
  size_t it;
  va_list args;

  wget_global_init (WGET_DEBUG_STREAM, stderr, WGET_ERROR_STREAM, stderr,
  ...
  wget_ssl_set_config_string (WGET_SSL_CA_FILE,
                              "../" SRCDIR "/certs/x509-ca-cert.pem");
  wget_ssl_set_config_string (WGET_SSL_CERT_FILE,
                              "../" SRCDIR "/certs/x509-server-cert.pem");
  wget_ssl_set_config_string (WGET_SSL_KEY_FILE,
  ...
  for (it = 0; it < nurls; it++)
    {
      if (urls[it].body
          && (strstr (urls[it].body, "{{port}}")
              || strstr (urls[it].body, "{{sslport}}")
              || strstr (urls[it].body, "{{ftpport}}")
              || strstr (urls[it].body, "{{ftpsport}}")))
        {
  ...
  if ((rc =
       wget_thread_start (&http_server_tid, _http_server_thread,
                          http_parent_tcp, 0)) != 0)
    wget_error_printf_exit (_("Failed to start HTTP server, error %d\n"), rc);

  // start thread for HTTPS
  if ((rc =
       wget_thread_start (&https_server_tid, _http_server_thread,
                          https_parent_tcp, 0)) != 0)
    wget_error_printf_exit (_("Failed to start HTTPS server, error %d\n"),
                            rc);

  // start thread for FTP
  if ((rc =
       wget_thread_start (&ftp_server_tid, _ftp_server_thread, ftp_parent_tcp,
                          0)) != 0)
  ...
  if (ftps_implicit)
    {
      if ((rc =
           wget_thread_start (&ftps_server_tid, _ftp_server_thread,
                              ftps_parent_tcp, 0)) != 0)
        wget_error_printf_exit (_("Failed to start FTP server, error %d\n"),
                                rc);
    }
...
static void
_scan_for_unexpected (const char *dirname,
                      const wget_test_file_t * expected_files)
{
  DIR *dir;
  struct dirent *dp;
  struct stat st;
          ...
          if (expected_files)
            {
              ...
              for (it = 0;
                   expected_files[it].name
                     && strcmp (expected_files[it].name, fname); it++);
...
void
wget_test (int first_key, ...)
{
  const char *request_url, *options = "", *executable = "../../src/wget2";
  const wget_test_file_t * expected_files = NULL, *existing_files = NULL;
  wget_buffer_t * cmd = wget_buffer_alloc (1024);
  unsigned it;
  int key, fd, rc, expected_error_code = 0;
  va_list args;
  ...
  if (existing_files)
    {
      for (it = 0; existing_files[it].name; it++)
        {
          if ((fd =
               open (existing_files[it].name, O_CREAT | O_WRONLY | O_TRUNC,
                     0644)) != -1)
            {
              ssize_t nbytes =
                write (fd, existing_files[it].content,
                       strlen (existing_files[it].content));
              close (fd);

              if (nbytes != (ssize_t) strlen (existing_files[it].content))
                wget_error_printf_exit (_
                                        ("Failed to write %zu bytes to file %s/%s [%s]\n"),
                                        strlen (existing_files[it].content),
                                        tmpdir, existing_files[it].name,
                                        options);

              if (existing_files[it].timestamp)
                {
                  // take the old utime() instead of utimes()
                  if (utime (existing_files[it].name, &(struct utimbuf)
                             {
                               0, existing_files[it].timestamp}))
                    wget_error_printf_exit (_
          ...
          else
            {
              wget_error_printf_exit (_
                                      ...
                                      tmpdir,
                                      *existing_files[it].name ==
                                      '/' ? existing_files[it].name +
                                      1 : existing_files[it].name, options,
  ...
  if (!valgrind || !*valgrind || !strcmp (valgrind, "0"))
    ...
    {
      wget_buffer_printf2 (cmd,
                           "valgrind --error-exitcode=301 --leak-check=yes --show-reachable=yes --track-origins=yes %s %s",
                           executable, options);
    }
  else
  ...
  for (it = 0; it < (size_t) wget_vector_size (request_urls); it++)
    {
      wget_buffer_printf_append2 (cmd, " 'http://localhost:%d/%s'",
                                  http_server_port,
                                  (char *) wget_vector_get (request_urls,
                                                            it));
    }
      ...
      for (it = 0; expected_files[it].name; it++)
        {
              ...
              if ((fd = open (expected_files[it].name, O_RDONLY)) != -1)
                {
                  ...
                  if (nbytes != st.st_size)
                    wget_error_printf_exit (_
                                            ("Failed to read %lld bytes from file %s/%s [%s]\n"),
                                            (long long) st.st_size, tmpdir,
                                            expected_files[it].name, options);

                  if (strlen (expected_files[it].content) != (size_t) nbytes
                      || memcmp (expected_files[it].content, content,
                                 nbytes) != 0)
                    wget_error_printf_exit (_
          ...
          if (expected_files[it].timestamp
              && st.st_mtime != expected_files[it].timestamp)
...
int
wget_test_get_http_server_port (void)
{
  return http_server_port;
}

int
wget_test_get_https_server_port (void)
{
  return https_server_port;
}

int
wget_test_get_ftp_server_port (void)
{
  return ftp_server_port;
}

int
wget_test_get_ftps_server_port (void)
{
