...
main (void)
{
  wget_test_url_t urls[] = {
    {.name = "/urls.txt",
     .code = "200 Dontcare",
     .body =
     "https://localhost:{{sslport}}/page1.html\nhttps://localhost:{{sslport}}/page2.html\n",
     .headers = {
  ...
  wget_test (
             ...
             WGET_TEST_OPTIONS,
             "--ca-certificate=../" SRCDIR
             "/certs/x509-ca-cert.pem --no-ocsp -i urls.txt",
             WGET_TEST_REQUEST_URL, NULL, WGET_TEST_EXPECTED_ERROR_CODE, 0,
  ...
  wget_test (
             ...
             WGET_TEST_OPTIONS,
             "--ca-certificate=../" SRCDIR
             "/certs/x509-ca-cert.pem --crl-file=../" SRCDIR
             "/certs/x509-server-crl.pem --no-ocsp -i urls.txt",
             WGET_TEST_REQUEST_URL, NULL, WGET_TEST_EXPECTED_ERROR_CODE, 5,
             WGET_TEST_EXISTING_FILES, &(wget_test_file_t[])
             {
               {
                 "urls.txt", urls[0].body},
  ...
  wget_test (
             // WGET_TEST_KEEP_TMPFILES, 1,
             WGET_TEST_OPTIONS,
             "--no-check-certificate --crl-file=../" SRCDIR
             "/certs/x509-server-crl.pem -i urls.txt", WGET_TEST_REQUEST_URL,
             NULL, WGET_TEST_EXPECTED_ERROR_CODE, 0,
             WGET_TEST_EXISTING_FILES, &(wget_test_file_t[])
             {
               {
                 "urls.txt", urls[0].body},
