...
static void
html_dump (void *user_ctx, int flags, const char *dir G_GNUC_WGET_UNUSED,
           const char *attr, const char *val, size_t len, size_t pos)
{
          ...
          if (memcmp (doc + pos, val, len))
            {
              failed++;
              error_printf_exit
                ("Not found: '%.*s' expected at pos %zd with length %zd\n",
                 (int) len, val, pos, len);
            }
          else
            ok++;
...
test_parse_buffer (void)
{
  ...
  for (it = 0; it < countof (test_data); it++)
    {
      wget_html_parse_buffer (test_data[it], html_dump,
                              (void *) test_data[it], 0);
    }
}

static void
test_parse_files (void)
{
  DIR *dirp;
  struct dirent *dp;
  const char *ext;
