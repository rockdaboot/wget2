...
_print_word (G_GNUC_WGET_UNUSED void *ctx, const char *word)
{
  printf ("%s\n", word);
  return 0;
}
...
main (int argc, const char *const *argv)
{
  ...
  for (it = 1; it < argc; it++)
    {
      ...
      if (!
          (buf =
           mmap (NULL, length + 1, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd,
                 0)))
        {
...
#else
      if (!(buf = xmalloc (length + 1))
          || read (fd, buf, length) != (signed) length)
        {
  ...
  wget_stringmap_browse (map,
                         (int (*)(void *, const char *, void *)) _print_word,
                         NULL);
