...
typedef struct {
	char
		dummy; // empty struct is a GNU extension and also not C++ compatible
} site_stats_t;
...
static void stats_print_site_human(G_GNUC_WGET_UNUSED stats_opts_t *opts, FILE *fp)
{
	fprintf(fp, "\nSite Statistics:\n");
...
static void stats_print_site_tree(G_GNUC_WGET_UNUSED stats_opts_t *opts, FILE *fp)
{
	stats_print_tree(fp);
}
