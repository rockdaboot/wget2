...
typedef struct
{
  ...
  const char *encoding;
  wget_iri_t *base_url;
  WGET_HTML_PARSED_RESULT *parsed;
  int content_type;
} _conversion_t;
...
save_file (wget_http_response_t * resp, const char *fname),
  ...
  sitemap_parse_xml (JOB * job, const char *data, const char *encoding,
                     wget_iri_t * base), sitemap_parse_xml_gz (JOB * job,
                                                               wget_buffer_t *
                                                               data,
                                                               const char
                                                               *encoding,
                                                               wget_iri_t *
                                                               base),
  sitemap_parse_xml_localfile (JOB * job, const char *fname,
                               const char *encoding, wget_iri_t * base),
  sitemap_parse_text (JOB * job, const char *data, const char *encoding,
                      wget_iri_t * base), atom_parse (JOB * job,
                                                      const char *data,
                                                      const char *encoding,
                                                      wget_iri_t * base),
  atom_parse_localfile (JOB * job, const char *fname, const char *encoding,
                        wget_iri_t * base), rss_parse (JOB * job,
                                                       const char *data,
                                                       const char *encoding,
                                                       wget_iri_t * base),
  rss_parse_localfile (JOB * job, const char *fname, const char *encoding,
                       wget_iri_t * base), html_parse (JOB * job, int level,
                                                       const char *data,
                                                       const char *encoding,
                                                       wget_iri_t * base),
  html_parse_localfile (JOB * job, int level, const char *fname,
                        const char *encoding, wget_iri_t * base),
  css_parse (JOB * job, const char *data, const char *encoding,
             wget_iri_t * base), css_parse_localfile (JOB * job,
                                                      const char *fname,
                                                      const char *encoding,
                                                      wget_iri_t * base);
static int download_part (DOWNLOADER * downloader);
static unsigned int G_GNUC_WGET_PURE hash_url (const char *url);
wget_http_response_t
* http_get (wget_iri_t * iri, PART * part, DOWNLOADER * downloader,
            const char *method);

static wget_stringmap_t * etags;
static wget_hashmap_t * known_urls;
static DOWNLOADER * downloaders;
static void *downloader_thread (void *p);
static long long quota;
static int exit_status, hsts_changed;
 ...
 */
static char *
restrict_file_name (char *fname, char *esc)
{
  char *s, *dst;
  int escaped, c;
      ...
      if (rc)
        {
          ...
          if (errno == EEXIST && stat (fname, &st) == 0
...
const char *G_GNUC_WGET_NONNULL_ALL
get_local_filename (wget_iri_t * iri)
{
  wget_buffer_t buf;
  char *fname;
  int directories;
  ...
  if (directories)
    {
      ...
      fname =
        wget_iri_get_query_as_filename (iri, &buf, config.local_encoding);
    }
  else
    {
...
static long long
quota_modify_read (size_t nbytes)
{
...
static wget_vector_t * parents;
static wget_thread_mutex_t downloader_mutex = WGET_THREAD_MUTEX_INITIALIZER;

static int
in_pattern_list (const wget_vector_t * v, const char *url)
{
...
static int
in_host_pattern_list (const wget_vector_t * v, const char *hostname)
{
  for (int it = 0; it < wget_vector_size (v); it++)
    {
      const char *pattern = wget_vector_get (v, it);
...
static void
add_url_to_queue (const char *url, wget_iri_t * base, const char *encoding)
{
  wget_iri_t *iri;
          ...
          if ((host = hosts_add (iri)))
            {
              ...
              new_job =
                job_init (&job_buf,
                          wget_iri_parse_base (iri, "/robots.txt", encoding));
              new_job->host = host;
...
static wget_thread_cond_t main_cond = WGET_THREAD_COND_INITIALIZER,  // is signalled whenever a job is done
  worker_cond = WGET_THREAD_COND_INITIALIZER;  // is signalled whenever a job is added
static wget_thread_t input_tid;
static void *input_thread (void *p);

// Needs to be thread-save
static void
add_url (JOB * job, const char *encoding, const char *url, int flags)
{
  JOB *new_job = NULL, job_buf;
  wget_iri_t *iri;
  ...
  if (flags & URL_FLG_REDIRECTION)
    {        // redirect
      if (config.max_redirect && job
          && job->redirection_level >= config.max_redirect)
        {
          return;
        }
    }
      ...
      for (int it = 0; it < wget_vector_size (parents); it++)
        {
          ...
          if (!strcmp (parent->host, iri->host))
            {
              if (!parent->dirlen
                  || !strncmp (parent->path, iri->path, parent->dirlen))
                {
                  // info_printf("found\n");
                  ok = 1;
                  break;
  ...
  if (config.recursive)
    {
      ...
      else if (!config.span_hosts && config.domains
               && !in_host_pattern_list (config.domains, iri->host))
        {
          reason = _("no host-spanning requested");
        }
      else if (config.span_hosts && config.exclude_domains
               && in_host_pattern_list (config.exclude_domains, iri->host))
        {
      ...
      if ((host = hosts_add (iri)))
      ...
      else if ((host = hosts_get (iri)))
        {
          ...
          if (host->robots && iri->path)
            {
              for (int it = 0; it < wget_vector_size (host->robots->paths);
                   it++)
                {
                  ROBOTS_PATH *path =
                    wget_vector_get (host->robots->paths, it);
  ...
  if (new_job)
    {
      if (!config.output_document)
        {
          if (!(flags & URL_FLG_REDIRECTION) || config.trust_server_names
              || !job)
            new_job->local_filename = get_local_filename (new_job->iri);
          else
  ...
  for (int it = 0; it < wget_vector_size (conversions); it++)
    {
      ...
      wget_info_printf ("convert %s %s %s\n", conversion->filename,
                        conversion->base_url->uri, conversion->encoding);

      if (!
          (data = data_ptr =
           wget_read_file (conversion->filename, &data_length)))
        {
      ...
      for (int it2 = 0; it2 < wget_vector_size (conversion->parsed->uris);
           it2++)
        {
          WGET_HTML_PARSED_URL *html_url =
            wget_vector_get (conversion->parsed->uris, it2);
          ...
          if (wget_iri_relative_to_abs
              (conversion->base_url, url->p, url->len, &buf))
            {
              // buf.data now holds the absolute URL as a string
              wget_iri_t *iri =
                wget_iri_parse (buf.data, conversion->encoding);
              ...
              if (access (filename, W_OK) == 0)
                {
                  ...
                  for (dir = p1 = linkpath, p2 = docpath; *p1 && *p1 == *p2;
                       p1++, p2++)
                    if (*p1 == '/')
                      dir = p1 + 1;
              ...
              if (buf.length != url->len
                  || strncmp (buf.data, url->p, url->len))
                {
                  ...
                  if (!fpout)
                    {
                      ...
                      if (!(fpout = fopen (conversion->filename, "w")))
                        wget_error_printf (_("Failed to write open %s (%d)"),
...
static void
print_status (DOWNLOADER * downloader, const char *fmt, ...)
  G_GNUC_WGET_NONNULL_ALL G_GNUC_WGET_PRINTF_FORMAT (2, 3);
static void print_status (DOWNLOADER * downloader G_GNUC_WGET_UNUSED,
                          const char *fmt, ...)
{
  if (config.verbose)
    {
...
#include <locale.h>
  ...
  known_urls =
    wget_hashmap_create (128, -2, (unsigned int (*)(const void *)) hash_url,
      ...
      if (config.force_html)
        {
          ...
          html_parse_localfile (NULL, 0, config.input_file,
                                config.input_encoding, config.base);
        }
      else if (config.force_css)
        {
          // read URLs from CSS file
          css_parse_localfile (NULL, config.input_file, config.input_encoding,
                               config.base);
        }
      else if (config.force_sitemap)
        {
          // read URLs from Sitemap XML file (base is normally not needed, all URLs should be absolute)
          sitemap_parse_xml_localfile (NULL, config.input_file, "utf-8",
                                       config.base);
        }
      else if (config.force_atom)
        {
          // read URLs from Atom Feed XML file
          atom_parse_localfile (NULL, config.input_file, "utf-8",
                                config.base);
        }
          ...
          if (isatty (STDIN_FILENO))
            {
              ...
              while ((len =
                      wget_fdgetline (&buf, &bufsize, STDIN_FILENO)) >= 0)
                {
          ...
          else
            {
              ...
              if ((rc =
                   wget_thread_start (&input_tid, input_thread, NULL,
                                      0)) != 0)
  ...
  config.num_threads =
    (config.max_threads < queue_size ())? config.max_threads : queue_size ();
  ...
  for (n = 0; n < config.num_threads; n++)
    {
      ...
      if ((rc =
           wget_thread_start (&downloaders[n].tid, downloader_thread,
                              &downloaders[n], 0)) != 0)
        {
          error_printf (_("Failed to start downloader, error %d\n"), rc);
        }
  ...
  while (!terminate)
    {
      ...
      if (config.progress)
        bar_printf (config.num_threads, "Files: %d/%d  Bytes: %llu",
                    blacklist_size () - queue_size (), blacklist_size (),
                    quota);
  ...
  if (config.progress)
    bar_printf (config.num_threads, "Files: %d/%d  Bytes: %llu",
                downloaded_files, blacklist_size (), quota);
  else
    if ((config.recursive || config.page_requisites
         || (config.input_file && quota != 0)) && quota)
      {
...
void *
input_thread (void *p G_GNUC_WGET_UNUSED)
{
  ssize_t len;
  size_t bufsize = 0;
  char *buf = NULL;
...
{
  static wget_thread_mutex_t etag_mutex = WGET_THREAD_MUTEX_INITIALIZER;
  ...
  while (!terminate)
    {
      ...
      if (config.accept_patterns
          && !in_pattern_list (config.accept_patterns, job->iri->uri))
        {
          if (config.recursive)
            job->head_first = 1;  // enable mime-type check to assure e.g. text/html to be downloaded and parsed
        }

      if (config.reject_patterns
          && in_pattern_list (config.reject_patterns, job->iri->uri))
        {
          if (config.recursive)
            job->head_first = 1;  // enable mime-type check to assure e.g. text/html to be downloaded and parsed
        }
      ...
      if ((config.spider || config.chunk_size || job->head_first)
          && !job->deferred)
        {
          ...
          for (int tries = 0; !resp && tries < config.tries; tries++)
            {
              wget_millisleep (tries * 1000 >
                               config.waitretry ? config.waitretry : tries *
                               1000);
          ...
          if (config.spider || job->head_first)
            {
              ...
              if (strcasecmp (resp->content_type, "text/html")
                  ...
                  && (!job->sitemap
                      || !strcasecmp (resp->content_type, "application/xml"))
                  && (!job->sitemap
                      || !strcasecmp (resp->content_type,
                                      "application/x-gzip")) && (!job->sitemap
                                                                 ||
                                                                 !strcasecmp
                                                                 (resp->
                                                                  content_type,
              ...
              if (resp->etag)
                {
                  ...
                  int rc =
                    wget_stringmap_put_noalloc (etags, resp->etag, NULL);
                  resp->etag = NULL;
                   ...
                   && resp->content_length > config.chunk_size)
            {
              ...
              wget_metalink_t *metalink =
                xcalloc (1, sizeof (wget_metalink_t));
              ...
              for (int it = 0; it < npieces; it++)
                {
                  ...
                  wget_vector_add (metalink->pieces, &piece,
                                   sizeof (wget_metalink_piece_t));
                }
      ...
      if (config.hsts && job->iri->scheme == WGET_IRI_SCHEME_HTTPS
          && resp->hsts)
        {
          wget_hsts_db_add (config.hsts_db,
                            wget_hsts_new (job->iri->host,
                                           atoi (job->iri->resolv_port),
                                           resp->hsts_maxage,
              ...
              if (link->rel == link_rel_describedby)
                {
                  if (link->type
                      &&
                      (!strcasecmp (link->type, "application/metalink4+xml")
                       || !strcasecmp (link->type,
      ...
      if (resp->content_type)
        {
          if (!strcasecmp (resp->content_type, "application/metalink4+xml"))
            ...
            if (!strcasecmp (resp->content_type, "application/metalink+xml"))
              {
      ...
      if (resp->code == 200)
        {
          if (config.content_disposition && resp->content_filename)
            ...
            save_file (resp,
                       config.output_document ? config.output_document : job->
                       local_filename);

          if (config.recursive
              && (!config.level
                  || job->level < config.level + config.page_requisites))
            {
              if (resp->content_type)
                {
                  if (!strcasecmp (resp->content_type, "text/html"))
                    {
                      html_parse (job, job->level, resp->body->data,
                                  resp->content_type_encoding ? resp->
                                  content_type_encoding : config.
                                  remote_encoding, job->iri);
                    }
                  else
                    if (!strcasecmp
                      ...
                      if (!strcasecmp
                        ...
                        {    // see RFC4287, http://de.wikipedia.org/wiki/Atom_%28Format%29
                        ...
                        if (!strcasecmp
                        ...
                        else if (job->sitemap)
                          {
                            if (!strcasecmp (resp->content_type, "application/xml"))
                              sitemap_parse_xml (job, resp->body->data, "utf-8",
                                                 job->iri);
                            else
                              if (!strcasecmp
                                  (resp->content_type, "application/x-gzip"))
                                sitemap_parse_xml_gz (job, resp->body, "utf-8",
                                                      job->iri);
                              else if (!strcasecmp (resp->content_type, "text/plain"))
                                sitemap_parse_text (job, resp->body->data, "utf-8",
                                                    job->iri);
                          }
                        else if (job->deferred
                                 && !strcasecmp (resp->content_type, "text/plain"))
                          {
                            debug_printf ("Scanning robots.txt ...\n");
                            if ((job->host->robots =
                                 wget_robots_parse (resp->body->data)))
                              {
                                // add sitemaps to be downloaded (format http://www.sitemaps.org/protocol.html)
                                for (int it = 0;
                                     it <
                                       wget_vector_size (job->host->robots->sitemaps);
                                     it++)
                                  {
                                    const char *sitemap =
                                      wget_vector_get (job->host->robots->sitemaps,
                                                       it);
      ...
      else if (resp->code == 206 && config.continue_download)
        {      // partial content
          if (config.content_disposition && resp->content_filename)
            append_file (resp, resp->content_filename);
          else
            append_file (resp,
                         config.output_document ? config.
                         output_document : job->local_filename);
        }
      else if (resp->code == 304 && config.timestamping)
        {      // local document is up-to-date
          if (config.recursive
              && (!config.level
                  || job->level < config.level + config.page_requisites)
              && job->local_filename)
              ...
              if (ext)
                {
                  if (!strcasecmp (ext, ".html") || !strcasecmp (ext, ".htm"))
                    {
                      html_parse_localfile (job, job->level,
                                            job->local_filename,
                                            resp->
                                            content_type_encoding ? resp->
                                            content_type_encoding : config.
                                            remote_encoding, job->iri);
                    }
                  else if (!strcasecmp (ext, ".css"))
                    {
                      css_parse_localfile (job, job->local_filename,
                                           resp->
                                           content_type_encoding ? resp->
                                           content_type_encoding : config.
                                           remote_encoding, job->iri);
...
static void
_free_conversion_entry (_conversion_t * conversion)
{
...
static void
_remember_for_conversion (const char *filename, wget_iri_t * base_url,
                          int content_type, const char *encoding,
                          WGET_HTML_PARSED_RESULT * parsed)
{
  static wget_thread_mutex_t mutex = WGET_THREAD_MUTEX_INITIALIZER;
  ...
  if (!conversions)
    {
      ...
      wget_vector_set_destructor (conversions,
                                  (void (*)(void *)) _free_conversion_entry);
    }
...
void
html_parse (JOB * job, int level, const char *html, const char *encoding,
            wget_iri_t * base)
{
  WGET_HTML_PARSED_RESULT *parsed =
    wget_html_get_urls_inline (html, config.follow_tags, config.ignore_tags);
  ...
  if (encoding && encoding == config.remote_encoding)
  ...
  else
    {
      ...
      else if ((unsigned char) html[0] == 0xFF
               && (unsigned char) html[1] == 0xFE)
        {
          // Little-endian UTF-16
          encoding = "UTF-16LE";
          reason = _("set by BOM");
        }
      else if ((unsigned char) html[0] == 0xEF
      ...
      if (!wget_strncasecmp (parsed->encoding, "UTF-16", 6)
          || !wget_strncasecmp (encoding, "UTF-16", 6))
        {
          // http://www.whatwg.org/specs/web-apps/current-work/, 12.2.2.2
  ...
  int page_requisites = config.recursive && config.page_requisites
  ...
  for (int it = 0; it < wget_vector_size (parsed->uris); it++)
    {
      ...
      if (wget_hashmap_put_noalloc
          (known_urls, strndup (url->p, url->len), NULL))
        {
      ...
      if (page_requisites && !strcasecmp (html_url->attr, "href"))
        {
          ...
          if (tolower (*html_url->dir) == 'a'
              && (html_url->dir[1] == 0 || !strcasecmp (html_url->dir, "area")
                  || !strcasecmp (html_url->dir, "embed")))
            {
              info_printf (_
  ...
  if (config.convert_links && !config.delete_after)
    {
      ...
      _remember_for_conversion (job->local_filename, base, _CONTENT_TYPE_HTML,
                                encoding, parsed);
  ...
  for (int it = 0; it < wget_vector_size (urls); it++)
    {
      ...
      if (baselen
          && (url->len <= baselen
              || strncasecmp (url->p, base->uri, baselen)))
        {
          info_printf (_
      ...
      // Blacklist for URLs before they are processed
      if (wget_hashmap_put_noalloc
          (known_urls, (p = strndup (url->p, url->len)), NULL))
        {
...
static int
_get_unzipped (void *userdata, const char *data, size_t length)
{
...
void
sitemap_parse_xml_gz (JOB * job, wget_buffer_t * gzipped_data,
                      const char *encoding, wget_iri_t * base)
{
  ...
  if ((dc =
       wget_decompress_open (wget_content_encoding_gzip, _get_unzipped,
                             plain)))
    {
      ...
      if (len)
        {
          ...
          // but not any other.
          if (baselen
              && (len <= baselen || strncasecmp (line, base->uri, baselen)))
            {
              info_printf (_
                           ("URL '%.*s' not followed (not matching sitemap location)\n"),
                           (int) len, line);
...
static void
_add_urls (JOB * job, wget_vector_t * urls, const char *encoding,
           wget_iri_t * base)
{
  const char *p;
  size_t baselen = 0;
...
void
atom_parse (JOB * job, const char *data, const char *encoding,
            wget_iri_t * base)
{
  wget_vector_t *urls;
...
void
rss_parse (JOB * job, const char *data, const char *encoding,
           wget_iri_t * base)
{
  wget_vector_t *urls;
...
struct css_context
{
  ...
  char encoding_allocated;
};

static void
_css_parse_encoding (void *context, const char *encoding, size_t len)
{
  struct css_context *ctx = context;

  // take only the first @charset rule
  if (!ctx->encoding_allocated
      && wget_strncasecmp (ctx->encoding, encoding, len))
    {
...
static void
_css_parse_uri (void *context, const char *url, size_t len,
           ...
           wget_iri_t * base)
{
  ...
  struct css_context context = {.base = base,.job = job,.encoding =
                                encoding };
  char sbuf[1024];
...
void
css_parse_localfile (JOB * job, const char *fname, const char *encoding,
                     wget_iri_t * base)
{
  // create scheme://authority that will be prepended to relative paths
  struct css_context context = {.base = base,.job = job,.encoding =
...
static long long G_GNUC_WGET_NONNULL_ALL
...
static time_t G_GNUC_WGET_NONNULL_ALL
get_file_mtime (const char *fname)
{
  struct stat st;
...
#ifdef HAVE_FUTIMENS
static void
set_file_mtime (int fd, time_t modified)
{
...
#else
static void
set_file_mtime (const char *filename, time_t modified)
{
...
G_GNUC_WGET_NONNULL ((1))
  _save_file (wget_http_response_t * resp, const char *fname, int flag)
{
  static wget_thread_mutex_t savefile_mutex = WGET_THREAD_MUTEX_INITIALIZER;
  ...
  if (config.quota)
    {
      if (quota_modify_read
          (config.save_headers ? resp->header->length +
           resp->body->length : resp->body->length) >= config.quota)
        {
  ...
  else
    {
      ...
      quota_modify_read (config.save_headers ? resp->header->length +
                         resp->body->length : resp->body->length);
    }
      ...
      if (!strcmp (fname, "-"))
        {
          ...
          if (config.save_headers)
            {
              if ((rc =
                   fwrite (resp->header->data, 1, resp->header->length,
                           stdout)) != resp->header->length)
                {
                  error_printf (_
          ...
          if ((rc =
               fwrite (resp->body->data, 1, resp->body->length,
                       stdout)) != resp->body->length)
            {
      ...
      if (ext)
        {
          ...
          if (fname_length >= ext_length
              && strcasecmp (fname + fname_length - ext_length, ext))
            {
  ...
  if (config.reject_patterns
      && in_pattern_list (config.reject_patterns, fname))
    {
  ...
  *unique = 0;
  for (fnum = 0, maxloop = 999;
       fd < 0 && ((multiple && errno == EEXIST) || errno == EISDIR)
         && fnum < maxloop; fnum++)
    {
  ...
  if (fd >= 0)
    {
      ...
      if (config.save_headers)
        {
          if ((rc =
               write (fd, resp->header->data,
                      resp->header->length)) !=
              (ssize_t) resp->header->length)
            {
      ...
      if ((rc =
           write (fd, resp->body->data,
                  resp->body->length)) != (ssize_t) resp->body->length)
        {
          error_printf (_("Failed to write file %s (%zd, errno=%d)\n"),
                        fnum ? unique : fname, rc, errno);
          set_exit_status (3);
...
download_part (DOWNLOADER * downloader)
{
  ...
  for (int tries = 0; tries < config.tries && !part->done; tries++)
    {
      wget_millisleep (tries * 1000 >
                       config.waitretry ? config.waitretry : tries * 1000);

      for (int mirrors = 0;
           mirrors < wget_vector_size (metalink->mirrors) && !part->done;
           mirrors++)
        {
          wget_http_response_t *resp;
          wget_metalink_mirror_t *mirror =
            wget_vector_get (metalink->mirrors, mirror_index);

          print_status (downloader,
                        "downloading part %d/%d (%lld-%lld) %s from %s (mirror %d)\n",
                        part->id, wget_vector_size (job->parts),
                        (long long) part->position,
                        (long long) (part->position + part->length - 1),
                        metalink->name, mirror->iri->host, mirror_index);

          mirror_index =
            (mirror_index + 1) % wget_vector_size (metalink->mirrors);
          ...
          if (resp)
            {
              ...
              if (resp->code != 200 && resp->code != 206)
                ...
                {
                  print_status (downloader,
                                "part %d download error '%zd bytes of %lld expected'\n",
                                part->id, resp->body->length,
                                (long long) part->length);
                }
              else
                {
                  int fd;
                  ...
                  if ((fd =
                       open (metalink->name, O_WRONLY | O_CREAT, 0644)) != -1)
                    {
#ifdef HAVE_PWRITE
                      ssize_t nbytes;
                      if ((nbytes =
                           pwrite (fd, resp->body->data, resp->body->length,
                                   part->position)) ==
                          (ssize_t) resp->body->length)
#else
                        ssize_t nbytes = -1;
                      if (fseek (fd, part->position, SEEK_SET) == 0
                          && (nbytes =
                              write (fd, resp->body->data,
                                     resp->body->length)) ==
                          (ssize_t) resp->body->length)
                        ...
                        error_printf (_
                                      ("Failed to pwrite %zd bytes at pos %lld (%zd)\n"),
                                      resp->body->length,
                                      (long long) part->position, nbytes);
...
static int
_get_header (void *context, wget_http_response_t * resp)
{
  struct _body_callback_context *ctx =
    (struct _body_callback_context *) context;

  // initialize the expected max. number of bytes for bar display
  bar_update (ctx->downloader->id, ctx->expected_length =
...
static int
_get_body (void *context, const char *data, size_t length)
{
  struct _body_callback_context *ctx =
    (struct _body_callback_context *) context;
          ...
          const char *method)
{
  ...
  if (config.hsts && iri && iri->scheme == WGET_IRI_SCHEME_HTTP
      && wget_hsts_host_match (config.hsts_db, iri->host,
                               atoi (iri->resolv_port)))
    {
  ...
  while (iri && ++tries <= config.tries)
    {
      ...
      if (conn && !wget_strcmp (conn->esc_host, iri->host) &&
      ...
      else
        {
          ...
          if ((rc =
               wget_http_open (&downloader->conn, iri)) == WGET_E_SUCCESS)
            {
      ...
      if (conn)
        {
                  ...
                  if (mtime)
                    {
                      ...
                      wget_http_print_date (mtime + 1, http_date,
                                            sizeof (http_date));
                      wget_http_add_header (req, "If-Modified-Since",
                                            http_date);
                    }
                }
            }
          ...
          */
          ...
          wget_buffer_strcat (&buf,
                              buf.
                              length ? ", gzip, deflate" : "gzip, deflate");
#endif
#if WITH_BZIP2
          ...
          if (challenges)
            {
              ...
              if (selected_challenge)
                wget_http_add_credentials (req, selected_challenge,
          ...
          if (config.cookies)
            {
              ...
              if ((cookie_string =
                   wget_cookie_create_request_header (config.cookie_db, iri)))
                {
          ...
          if (config.post_data)
            {
              ...
              wget_http_add_header (req, "Content-Type",
                                    "application/x-www-form-urlencoded");
              wget_http_add_header_printf (req, "Content-Length", "%zu",
                                           length);
              rc =
                wget_http_send_request_with_body (conn, req, config.post_data,
                                                  length);
            }
              ...
              if ((data = wget_read_file (config.post_file, &length)))
                {
                  ...
                  wget_http_add_header_printf (req, "Content-Length", "%zu",
                                               length);
                  rc =
                    wget_http_send_request_with_body (conn, req, data,
                                                      length);
                  xfree (data);
                }
              else
              ...
              if (config.progress)
                {
                  ...
                  struct _body_callback_context context = {.downloader =
                                                           downloader,.body = body };

                  resp =
                    wget_http_get_response_func (conn, _get_header, _get_body,
                                                 &context, config.save_headers
                                                 || config.
                                                 server_response ?
                ...
                resp =
                  wget_http_get_response (conn, NULL, req, config.save_headers
                                          || config.
                                          server_response ?
                                          WGET_HTTP_RESPONSE_KEEPHEADER : 0);
            }
      ...
      if (resp->location)
        {
          ...
          wget_iri_relative_to_abs (iri, resp->location,
                                    strlen (resp->location), &uri_buf);

          if (!part)
            {
              add_url (downloader->job, "utf-8", uri_buf.data,
                       URL_FLG_REDIRECTION);
