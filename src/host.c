...
static wget_hashmap_t * hosts;
static wget_thread_mutex_t hosts_mutex = WGET_THREAD_MUTEX_INITIALIZER;

static int
_host_compare (const HOST * host1, const HOST * host2)
{
  int n;
...
hosts_add (wget_iri_t * iri)
{
  ...
  if (!hosts)
    {
      hosts =
        wget_hashmap_create (16, -2,
                             (unsigned int (*)(const void *)) _host_hash,
                             (int (*)(const void *, const void *))
                             _host_compare);
      wget_hashmap_set_key_destructor (hosts,
                                       (void (*)(void *)) _free_host_entry);
    }
  ...
  if (!wget_hashmap_contains (hosts, &host))
    {
      ...
      wget_hashmap_put_noalloc (hosts, hostp =
                                wget_memdup (&host, sizeof (host)), NULL);
    }
