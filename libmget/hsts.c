...
// save the HSTS db to a flat file
...
int mget_hsts_db_save(mget_hsts_db_t *hsts_db, const char *fname)
{
	...
	if ((fp = fopen(fname, "w"))) {
		...
		size = mget_hashmap_size(hsts_db->entries);
		...
		if (ret)
			...
			info_printf(_("saved %d HSTS entr%s into '%s'\n"), size, size != 1 ? "ies" : "y", fname);
	} else
...
int mget_hsts_db_load(mget_hsts_db_t *hsts_db, const char *fname)
{
	...
	if ((fp = fopen(fname, "r"))) {
		...
		info_printf(_("found %d HSTS entr%s in '%s'\n"),
			mget_hashmap_size(hsts_db->entries), mget_hashmap_size(hsts_db->entries) !=1 ? "ies" : "y", fname);
	} else
		error_printf(_("Failed to open HSTS file '%s'\n"), fname);
