...
// save the OCSP db to a flat file
...
int mget_ocsp_db_save(mget_ocsp_db_t *ocsp_db, const char *fname)
{
	...
	if ((fp = fopen(fname, "w"))) {
		...
		size = mget_hashmap_size(ocsp_db->entries);
...
int mget_ocsp_db_load(mget_ocsp_db_t *ocsp_db, const char *fname)
{
	...
	if ((fp = fopen(fname, "r"))) {
		...
		info_printf(_("found %d OCSP entr%s in '%s'\n"),
			mget_hashmap_size(ocsp_db->entries), mget_hashmap_size(ocsp_db->entries) !=1 ? "ies" : "y", fname);
	} else
		error_printf(_("Failed to open OCSP file '%s'\n"), fname);
